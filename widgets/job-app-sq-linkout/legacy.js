...
$('input[type=radio][name=uploadTypeCover]')
    .on('change', function(){
    ...
    return shazamme.site().then( s =>
        s.isLive
            && shazamme.submit({
                ...
                r?.status
                && r?.response?.items?.find( x =>
                    x.jobID === getJobID()
                    && x.jobApplicationID !== new URL(window.location).searchParams.get('jobApplicationID')
                )
            )
...
function screeningQuestions(w, edit) {
    ...
    this._fetchQuestions = () => {
        ...
        return new Promise( resolve => {
            ...
            let fetchTemplate = new Promise( (resolve, reject) => {
                if (data.inEditor && !jobID) {
                    resolve(JSON.parse(sender._mock));
                    return;
                }
                        ...
                        .then( j => {
                            ...
                            if (screeningTemplateID) {
                                ...
                                collections
                                    ...
                                    .where('screeningTemplateID', 'EQ', screeningTemplateID)
                                    .get()
                                    .then( q => {
                                        resolve({
                                            response: {
    ...
    this._showQuestions = (page, scrollIntoView=true) => {
        ...
        this._recordAnswers().then( () => {
            ...
            let el = p.map( q => sender._questionElement({
                    ...q,
    ...
    this._questionElement = (q) => {
        ...
        switch (q.questionType) {
            ...
            case 'Multiselect Checkbox': {
                let opts = q.options?.map( o => `<label><input tabindex="30" type="checkbox" autocomplete="nope" data-qtype="check-list" data-qid="${q.screeningQuestionID}" data-value="${o.screeningQuestionOptionsID}" ${q.isMandatory ? 'required' : ''} />${o.label || o.option}</label>`);
            ...
            case 'Radio': {
                let opts = q.options?.map( o => `<label class="sq-question-option"><input tabindex="30" type="radio" data-qtype="radio" name="${q.screeningQuestionID}" data-qid="${q.screeningQuestionID}" value="${o.screeningQuestionOptionsID}" ${q.isMandatory ? 'required' : ''} />${o.label || o.option}</label>`);
    ...
    w.sub(Message.submit, () => {
        ...
        screening.answers().then( answers => {
            ...
            let apply = (u) => {
                ...
                // send (~64KB cap), so with files we keep the reliable awaited
                // submit and redirect once it resolves. Without files we fire the
                // record with keepalive — it still completes as the page navigates
                // away — and redirect IMMEDIATELY, so the loader doesn't sit on the
                // slow legacy backend. Referral/UTM tracking is in `a` either way.
                if (a.resumeFile || a.coverLetterFile) {
                    shazamme.submit(applyPayload).then( () => {
                        ...
                        // The awaited path is the only one that can see a failure;
                        // stock left the loader spinning forever here.
                        buttonAction("hide");
            ...
            // Fast link-out soft capture. The stock flow looked the user up and
            // then either duplicate-checked + edited the candidate (logged in) or
            // ran a firebase register (logged out) BEFORE applying — each a slow
            // backend/firebase round trip that keeps the loader spinning even
            // though apply() is now instant. A capture-then-link-out needs none of
            // it: mint a candidate from the form values and record it.
            //   NOTE: this intentionally skips login/register and duplicate
            //   checking. A logged-in candidate gets a fresh soft-capture record
            //   rather than an update to their account — correct for a soft
            //   application whose real apply happens at the link-out destination.
    ...
    jobRow()
        .then( j => {
            if (j?.data) {
                ...
                // for the external site before we captured anything. Here the
                // candidate fills the form and we redirect after submit — see the
                // link-out destination in apply() below.
