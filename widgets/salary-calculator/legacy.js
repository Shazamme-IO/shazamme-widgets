...
// === Hours-per-Day (editable) — additive, backward-compatible ==============
// Recompute the daily/fortnightly/monthly/yearly hour multipliers from the
// user's "Hours per Day" input while PRESERVING the configured working-day
// ratios. When the input equals the configured daily hours (the default), the
// multipliers equal the original config values, so existing AUS/UK/US calcs are
// unchanged until a user edits the field.
function recomputeHours() {
    var cfgD, cfgF, cfgM, cfgY;
    if (locations == "aus") {
        cfgD = data.config.ausdailyhourtxt; cfgF = data.config.ausfornightlyhourtxt;
        cfgM = data.config.ausmonthlyhourtxt; cfgY = data.config.ausyearlyhourtxt;
    } else {
        cfgD = data.config.dailyhourtxt; cfgF = data.config.fornightlyhourtxt;
        cfgM = data.config.monthlyhourtxt; cfgY = data.config.yearlyhourtxt;
    }
    var baseDaily = parseFloat(cfgD); if (isNaN(baseDaily) || baseDaily <= 0) baseDaily = 8;
    ...
    if (locations == "aus") {
        ausdailyhrs = H; ausfornightyhrs = H * dF; ausmonthlyhrs = H * dM; ausyearlyhrs = H * dY;
    } else {
        dailyhrs = H; fornightyhrs = H * dF; monthlyhrs = H * dM; yearlyhrs = H * dY;
    }
}

// Prefill the Hours-per-Day field with the configured daily hours (display only;
// no effect on any math until the user changes it).
...
// When the user edits Hours per Day, recompute the multipliers and re-run the
...
if(locations=="aus"){
    ...
    if (Layouts == "layout1"){
		if(data.device=="mobile"){
			...
			$(document).ready(function() {
				...
				$('#hourlyRatemobile,#payrollvalmobile,#annuationvalmobile').keyup(function() {
					...
					//var taxableHourlyRate = hourlyRate - payrollFeeAmount - superAmount;
					...
					if(hourlyRate!= null && hourlyRate!= undefined && hourlyRate!="" && !isNaN(hourlyRate)){
					...
					// document.getElementById('lassannuationval').value = superAmount.toFixed(2);
					...
					//var taxableYearlyRate = taxableDailyRate * 46;
					...
					//var superYearlyAmount = superAmount * 46;
					...
					if(BaseHourRate!= null && BaseHourRate!= undefined && BaseHourRate!="" && !isNaN(BaseHourRate)){
						//document.getElementById('hourlyResult').innerText = Math.round(taxableHourlyRate);
					...
					if(taxableDailyRate!= null && taxableDailyRate!= undefined && taxableDailyRate!="" && !isNaN(taxableDailyRate)){
						//document.getElementById('dailyResult').innerText = Math.round(taxableDailyRate);
					...
					if(taxableFortnightlyRate!= null && taxableFortnightlyRate!= undefined && taxableFortnightlyRate!="" && !isNaN(taxableFortnightlyRate)){
						//document.getElementById('fortnightlyResult').innerText = Math.round(taxableFortnightlyRate);
					...
					if(taxableMonthlyRate!= null && taxableMonthlyRate!= undefined && taxableMonthlyRate!="" && !isNaN(taxableMonthlyRate)){
						//document.getElementById('monthlyResult').innerText = Math.round(taxableMonthlyRate);
					...
					if(taxableYearlyRate!= null && taxableYearlyRate!= undefined && taxableYearlyRate!="" && !isNaN(taxableYearlyRate)){
						//document.getElementById('yearlyResult').innerText = Math.round(taxableYearlyRate);
					...
					if(superHourlyAmount!= null && superHourlyAmount!= undefined && superHourlyAmount!="" && !isNaN(superHourlyAmount)){
						//document.getElementById('hourlySuper').innerText = Math.round(superAmount);
					...
					if(superDailyAmount!= null && superDailyAmount!= undefined && superDailyAmount!="" && !isNaN(superDailyAmount)){
						//document.getElementById('dailySuper').innerText = Math.round(superDailyAmount);
					...
					if(superFortnightlyAmount!= null && superFortnightlyAmount!= undefined && superFortnightlyAmount!="" && !isNaN(superFortnightlyAmount)){
						//document.getElementById('fortnightlySuper').innerText = Math.round(superFortnightlyAmount);
					...
					if(superMonthlyAmount!= null && superMonthlyAmount!= undefined && superMonthlyAmount!="" && !isNaN(superMonthlyAmount)){
						//document.getElementById('monthlySuper').innerText = Math.round(superMonthlyAmount);
					...
					if(superYearlyAmount!= null && superYearlyAmount!= undefined && superYearlyAmount!="" && !isNaN(superYearlyAmount)){
						//document.getElementById('yearlySuper').innerText = Math.round(superYearlyAmount);
					...
					if(totalHourly!= null && totalHourly!= undefined && totalHourly!="" && !isNaN(totalHourly)){
						//document.getElementById('hourlyTotal').innerText =Math.round(parseFloat(taxableHourlyRate) + parseFloat(superAmount));
					...
					if(totalDaily!= null && totalDaily!= undefined && totalDaily!="" && !isNaN(totalDaily)){
						//document.getElementById('dailyTotal').innerText = Math.round(totalDaily);
					...
					if(totalFortnightly!= null && totalFortnightly!= undefined && totalFortnightly!="" && !isNaN(totalFortnightly)){
						//document.getElementById('fortnightlyTotal').innerText = Math.round(totalFortnightly);
					...
					if(totalMonthly!= null && totalMonthly!= undefined && totalMonthly!="" && !isNaN(totalMonthly)){
						//document.getElementById('monthlyTotal').innerText = Math.round(totalMonthly);
					...
					if(totalYearly!= null && totalYearly!= undefined && totalYearly!="" && !isNaN(totalYearly)){
						//document.getElementById('yearlyTotal').innerText = Math.round(totalYearly);
				...
				$('#currentannualmobile,#addloadmobile,#payrollval1mobile,#annuationval1mobile').keyup(function() {
							...
							//var annualPercentage = curannaualval / 1840.26;
							...
							//annualPercentage = annualPercentage.toFixed(2);
							document.getElementById('secondhourlymobile').value = annualPercentage.toFixed(2);
							/*var loadpercentage = (25/100)+1;
							var hourlyval = parseFloat(annualPercentage) * parseFloat(loadpercentage);
							hourlyval = hourlyval.toFixed(2);*/
							...
							//document.getElementById('hourlyid1').value = secondtaxableHourlyRate;
							...
							//var taxhour = parseFloat(totadd) - parseFloat(TotalCost);
							debugger
							...
							if(hourlyRate!= null && hourlyRate!= undefined && hourlyRate!="" && !isNaN(hourlyRate)){
								//document.getElementById('hourlyResult1').innerText = Math.round(taxhour);
							...
							if(taxableDailyRate!= null && taxableDailyRate!= undefined && taxableDailyRate!="" && !isNaN(taxableDailyRate)){
								//document.getElementById('dailyResult1').innerText = Math.round(taxableDailyRate);
							...
							if(taxableFortnightlyRate!= null && taxableFortnightlyRate!= undefined && taxableFortnightlyRate!="" && !isNaN(taxableFortnightlyRate)){
								//document.getElementById('fortnightlyResult1').innerText = Math.round(taxableFortnightlyRate);
							...
							if(taxableMonthlyRate!= null && taxableMonthlyRate!= undefined && taxableMonthlyRate!="" && !isNaN(taxableMonthlyRate)){
								//document.getElementById('monthlyResult1').innerText = Math.round(taxableMonthlyRate);
							...
							if(taxableYearlyRate!= null && taxableYearlyRate!= undefined && taxableYearlyRate!="" && !isNaN(taxableYearlyRate)){
								//document.getElementById('yearlyResult1').innerText = Math.round(taxableYearlyRate);
							...
							if(superAmount!= null && superAmount!= undefined && superAmount!="" && !isNaN(superAmount)){
								//document.getElementById('hourlySuper1').innerText = Math.round(superAmount);
							...
							if(superDailyAmount!= null && superDailyAmount!= undefined && superDailyAmount!="" && !isNaN(superDailyAmount)){
								//document.getElementById('dailySuper1').innerText = Math.round(superDailyAmount);
							...
							if(superFortnightlyAmount!= null && superFortnightlyAmount!= undefined && superFortnightlyAmount!="" && !isNaN(superFortnightlyAmount)){
								//document.getElementById('fortnightlySuper1').innerText = Math.round(superFortnightlyAmount);
							...
							if(superMonthlyAmount!= null && superMonthlyAmount!= undefined && superMonthlyAmount!="" && !isNaN(superMonthlyAmount)){
								//document.getElementById('monthlySuper1').innerText = Math.round(superMonthlyAmount);
							...
							if(superYearlyAmount!= null && superYearlyAmount!= undefined && superYearlyAmount!="" && !isNaN(superYearlyAmount)){
								//document.getElementById('yearlySuper1').innerText = Math.round(superYearlyAmount);
							...
							if(HourlyTotalPack!= null && HourlyTotalPack!= undefined && HourlyTotalPack!="" && !isNaN(HourlyTotalPack)){
								//document.getElementById('hourlyTotal1').innerText =Math.round(parseFloat(taxhour) + parseFloat(superAmount));
							...
							if(totalDaily!= null && totalDaily!= undefined && totalDaily!="" && !isNaN(totalDaily)){
								//document.getElementById('dailyTotal1').innerText = Math.round(totalDaily);
							...
							if(totalFortnightly!= null && totalFortnightly!= undefined && totalFortnightly!="" && !isNaN(totalFortnightly)){
								//document.getElementById('fortnightlyTotal1').innerText = Math.round(totalFortnightly);
							...
							if(totalMonthly!= null && totalMonthly!= undefined && totalMonthly!="" && !isNaN(totalMonthly)){
								//document.getElementById('monthlyTotal1').innerText = Math.round(totalMonthly);
							...
							if(totalYearly!= null && totalYearly!= undefined && totalYearly!="" && !isNaN(totalYearly)){
								//document.getElementById('yearlyTotal1').innerText = Math.round(totalYearly);
        ...
        if(data.device!="mobile"){
            ...
            function updateBars() {
                debugger
                const takeHome = parseFloat(document.getElementById("yearlyResult")?.textContent) || 0;
                const totalBeforeTax = parseFloat(document.getElementById("yearlyTotal")?.textContent) || 0;
                const superAmt = parseFloat(document.getElementById("yearlySuper")?.textContent) || 0;
                debugger
                ...
                // document.getElementById("takeHomeLabel").textContent = `$${takeHome.toLocaleString()}`;
                // document.getElementById("taxesLabel").textContent = `$${superAmt.toLocaleString()}`;
            ...
            var previousValue = "";
                ...
                $('.timelabel').on('click', function () {
                    ...
                    calculateSalaryFromLabel(selectedPeriod);
                ...
                function calculateSalaryFromLabel(period) {
                ...
                function calculateSalary() {
                    ...
                    if(MobileTotal != null && MobileTotal != undefined && MobileTotal != "" && !isNaN(MobileTotal)){
                    ...
                    if(MobileSuperAmount != null && MobileSuperAmount != undefined && MobileSuperAmount != "" && !isNaN(MobileSuperAmount)){
                    ...
                    if(MobileBaseRate != null && MobileBaseRate != undefined && MobileBaseRate != "" && !isNaN(MobileBaseRate)){
                ...
                function updateBars() {
                    const takeHome = parseFloat(document.getElementById("baseSalaryValue")?.textContent) || 0;
                    const totalBeforeTax = parseFloat(document.getElementById("totalPackageValue")?.textContent) || 0;
                    const superAmt = parseFloat(document.getElementById("superValue")?.textContent) || 0;
