 ...
 * Build f980880667f9. Registers window.ShazammeWidget["<name>"].
 */
var __shazWidgetExport=(()=>{var z=Object.defineProperty;var Oe=Object.getOwnPropertyDescriptor;var he=Object.getOwnPropertyNames;var me=Object.prototype.hasOwnProperty;var ve=(D,t)=>{for(var f in t)z(D,f,{get:t[f],enumerable:!0})},ye=(D,t,f,i)=>{if(t&&typeof t=="object"||typeof t=="function")for(let l of he(t))!me.call(D,l)&&l!==f&&z(D,l,{get:()=>t[l],enumerable:!(i=Oe(t,l))||i.enumerable});return D};var we=D=>ye(z({},"__esModule",{value:!0}),D);var Ce={};ve(Ce,{default:()=>fe});var Te="https://sdk.shazamme.io/js/shazamme-1.0.3.min.js",Se=/shazamme(-1\.0\.\d+(-test)?)?\.min\.js/;function ce(D){return new Promise((t,f)=>{let i=document.createElement("script");i.src=D,i.onload=()=>t(),i.onerror=()=>f(new Error(`Failed to load ${D}`)),(document.head||document.documentElement).appendChild(i)})}function Pe(){if(window.__shazSDKPromise)return window.__shazSDKPromise;let D=window.shazamme?Promise.resolve():ce(Te);return window.__shazSDKPromise=D,D}function ke(D){let t=window.__shazScriptCache=window.__shazScriptCache||{},f=t[D];if(f)return f;let i=ce(D).catch(l=>{throw delete t[D],l});return t[D]=i,i}function xe(){typeof window=="undefined"||window.__shazLoadScript||(window.__shazLoadScript=D=>Se.test(D)?Pe():ke(D))}function de(){xe()}function fe(D){de();var t=D.data,f=D.element,i=D.$||window.jQuery||window.$,l=D.shazamme||window.shazamme;let M=new ue,S={},Ee=!1,Le=!1,E=t.siteId,$e={},L="thank-you",Z="dashboard",Me="register",G;t.config.toggleThankyouPage&&(L=t.config.txt_Thankyoupageurl);let k=t.config.showResume===!0,A=t.config.applicationLayout!=="simpleForm"&&t.config.showCoverLetter,W=t.config.showSubscription==="true",_=new URL(window.location.href),F=!1,$=document.getElementById("passwordInput"),J={submit:"application-screening-form-submit"},H=(parseInt(t.config.maxUploadSize)||10)*1024*1024;if(t.config.showScreeningQuestions&&ne(),i("#passwordInput").focus(function(){i("#passwordMessage,.arrow-bottom").css("display","flex")}),i("#passwordInput").blur(function(){F&&i("#passwordMessage, .arrow-bottom").css("display","none")}),t.config.useSmartLogin&&!le()){var q=null;i(f).find(".smart-login-login, .smart-login-register").hide(),i("#emailLogin").keyup(function(){q&&clearTimeout(q),q=setTimeout(()=>{let s=i(this).val().trim();if(s.length===0){i(f).find(".smart-login-login, .smart-login-register").hide();return}firebase.auth().fetchSignInMethodsForEmail(s).then(c=>{i(f).find(".smart-login-login, .smart-login-register").hide(),c.length>0?i(f).find(".smart-login-login").show():(i(f).find(".smart-login-register").show(),i("#emailAddress").val(s))}).catch(()=>{i(f).find(".smart-login-login").hide(),i(f).find(".smart-login-register").show()}),q=null},500)})}if($){let s=document.getElementById("capital"),c=document.getElementById("number"),o=document.getElementById("length"),n=document.getElementById("specialChar");$.onkeyup=function(){let a=0;if($.value.length<8?(o.classList.remove("valid"),o.classList.add("invalid"),a-=1):(o.classList.remove("invalid"),o.classList.add("valid"),a+=1),$.value.search(/(?=.*[A-Z])/)!=-1?(s.classList.remove("invalid"),s.classList.add("valid"),status=!0,a+=1):(s.classList.remove("valid"),s.classList.add("invalid"),a-=1),$.value.search(/\d/)!=-1?(c.classList.remove("invalid"),c.classList.add("valid"),a+=1):(c.classList.remove("valid"),c.classList.add("invalid"),a-=1),$.value.search(/(?=.*[!@#$%^&*])/)!=-1?(n.classList.remove("invalid"),n.classList.add("valid"),a+=1):(n.classList.remove("valid"),n.classList.add("invalid"),a-=1),a===4){i(".passwordErrorTitle").text("Your password is acceptable!"),i("#passwordMessage").css("background-color","#59db5d"),i(".valid,.passwordErrorTitle").css("color","#fff"),i(".arrow-bottom").css({"border-top":"15px solid #59db5d"}),F=!0;return}else return i(".passwordErrorTitle").text("Password must contain the following"),i("#passwordMessage").css("background-color","#ffa1a1"),i("#passwordMessage,.passwordErrorTitle, .invalid").css("color","red"),i(".arrow-bottom").css({"border-top":"15px solid #ffa1a1"}),F=!1,!1}}function U(s,c){var o=s.files[0];let n=o.name.replace(/[^a-z0-9-_.]/gi,"-").replace(/-{2,}/gi,"-");var a=new FileReader;let e=c=="resume"?"resumeFile":"coverLetterFile",r=c=="resume"?"resumeName":"coverLetterFileName";a.addEventListener("load",function(){S[e]=a.result,S[r]=n},!1),o&&a.readAsBinaryString(o)}i(f).find("input#resume").on("change",function(){var n;let s=this;if(((n=s.files[0])==null?void 0:n.size)>H){alert(t.config.resumeSizeWarning||`Error! File too large, Should not exceed ${t.config.maxUploadSize||""}mb`),i(s).val("");return}let o=i("input[type=radio][name=uploadTypeResume][value=uploadResume]");o.get(0).checked=!0,o.trigger("change"),U(s,"resume")}).each(function(){var n;let s=this,c=(n=s.files[0])==null?void 0:n.size;if(s.files.length==0)return;if(c>H){alert(t.config.resumeSizeWarning||`Error! File too large, Should not exceed ${t.config.maxUploadSize||""}mb`),i(s).val("");return}let o=i("input[type=radio][name=uploadTypeResume][value=uploadResume]");o.get(0).checked=!0,o.trigger("change"),U(s,"resume")}),i(f).find("input#cover").on("change",function(){var n;let s=this;if(((n=s.files[0])==null?void 0:n.size)>H){alert(t.config.resumeSizeWarning||`Error! File too large, Should not exceed ${t.config.maxUploadSize||""}mb`),i(s).val("");return}let o=i("input[type=radio][name=uploadTypeCover][value=uploadCover]");o.get(0).checked=!0,o.trigger("change"),U(s,"cover")}).each(function(){var n;let s=this,c=(n=s.files[0])==null?void 0:n.size;if(s.files.length==0)return;if(c>H){alert(t.config.resumeSizeWarning||`Error! File too large, Should not exceed ${t.config.maxUploadSize||""}mb`),i(s).val("");return}let o=i("input[type=radio][name=uploadTypeCover][value=uploadCover]");o.get(0).checked=!0,o.trigger("change"),U(s,"cover")}),i("input[type=radio][name=uploadTypeResume]").change(function(){this.value=="uploadResume"?(i(".fileUploadContainer").css("display","flex"),k=!0):(i(".fileUploadContainer").css("display","none"),k=!1)}),i("input[type=radio][name=uploadTypeCover]").on("change",function(){this.value=="uploadCover"?(i(".coverUploadContainer").css("display","flex"),A=!0):(i(".coverUploadContainer").css("display","none"),A=!1)}).first().attr("checked","checked").trigger("change"),i("#loginBtn").click(function(){let s=i("#emailLogin").val(),c=i("#passwordLogin").val();G="loginForm",M.nativeLogin(s,c)}),i(".container-item").click(function(){switch(i(".registerTxt").hide(),i(this).attr("data-provider")){case"facebook":{M.loginOrRegisterUsingProvider(facebookProvider);break}case"google":{M.loginOrRegisterUsingProvider(googleProvider);break}case"linkedin":{l.oauth("linkedinProvider");break}case"seek":{l.oauth("seekProvider");break}}});function Y(s,c,o){if(c){let n=o=="resume"?t.config.warningResume||"Please upload your resume":t.config.warningCoverLetter||`Please upload your cover letter, or tick the "I don't have cover letter"`;if(s.files[0]==null){throw T("stop"),i(s).find(".fcLoader").removeClass("fcLoadingSeek"),alert(n),new Error(n);return null}return s.files[0]}return null}function Ae(s,c){s&&(S[c]=s)}function ue(){this.clearKeys=()=>{let s=["seekAuthorizationCode","authProvider","currentJobViewed","linkedIncode","jobID"];for(let c of s)l.store(c,null)},this.nativeLogin=(s,c)=>{l.firebase().auth(s,c).then(()=>{if(t.inEditor)return;if(G=="loginForm"){window.location.reload();return}if(G=="registrationForm"){let n=window.location.href.includes(E)?`/site/${E}/${L}?preview=true&insitepreview=true&dm_device=desktop`:`/${L}`;window.location.href=n;return}let o=l.store("previousApplicationPage");if(o){window.location.href=o;return}window.location.href=window.location.href.includes(E)?`/site/${E}/${L}?preview=true&insitepreview=true&dm_device=desktop`:`/${L}`}).catch(o=>{alert(o.msg)})},this.loginOrRegisterUsingProvider=s=>{l.firebase().oauth(s).then(c=>{c.isNew?l.submit({action:"Register Candidate",salutation:" ",firstName:c.firstName,surname:c.lastName,eMail:c.email,password:l.uuid(),candidateID:l.uuid(),firebaseUserID:c.firebaseUserID,isActive:!0,isValidated:!0,isSubscribed:W&&i(f).find("[data-rel=checkbox-subscribe]:checked").length>0,dudaSiteID:t.siteId}).then(o=>{o.status&&!t.inEditor&&l.auth(c.email,c.firebaseUserID,!0).then(()=>{window.location.reload()})}):window.location.reload()}).catch(c=>{alert(c.msg)})}}function K(s){let c=i(f);s?(c.find("#salutation").val(s.salutation||""),c.find("#firstName").val(s.firstName||""),c.find("#lastName").val(s.surname||""),c.find("#emailAddress").val(s.eMail||"").attr("readonly","readonly"),c.find("#phoneNumber").val(s.phone||""),c.find("#mobileNumber").val(s.mobile||""),s.isNew?(c.find(".shmLoginContainer,.applyMainSocial,.subscriptionContainer").show(),c.find(".greeting-message").hide()):(c.find(".shmLoginContainer,.applyMainSocial,.subscriptionContainer").hide(),c.find(".greeting-message").show().text(`${t.config.greeting||"Welcome back"} ${s.firstName||""} ${s.surname||""}!`))):(c.find(".shmregistrationContainer,.shmLoginContainer,.applyMainSocial,.subscriptionContainer").show(),c.find(".greeting-message").hide().text(""),c.find("#salutation").val(""),c.find("#firstName").val(""),c.find("#lastName").val(""),c.find("#emailAddress").val("").removeAttr("readonly"),c.find("#phoneNumber").val(""),c.find("#mobileNumber").val("")),t.inEditor&&c.find(".greeting-message").show().text(`${t.config.greeting||"Welcome back"}!`),s!=null&&s.cVFileContent||i(f).find(".uploadExisting").css("display","none"),t.config.showScreeningQuestions&&ne()}function le(){return window.localStorage.vinylResponse}let be=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i,V=null,R=null;function X(){let s=new URL(window.location.href);for(let[c,o]of s.searchParams.entries())if(c.toLowerCase()==="jobid"&&(o==null?void 0:o.length)>0)return o;return t.config.useSingleJob&&t.config.jobID||l.store("jobID")}function N(){return V||X()}function ee(){if(R)return R;let s=X();return R=l.site().then(c=>{if(!((s==null?void 0:s.length)>0))return null;if(be.test(s))return l.fetch({path:`/job-results/${c.siteID}/${s}`,isExternal:!0,useCache:!0});let o=String(s).toLowerCase().replace(/\/+$/,"").split("/").pop();return l.fetch({path:`/job-results/${c.siteID}`,isExternal:!0,useCache:!0}).then(n=>(n||[]).find(a=>{var r;let e=(r=a==null?void 0:a.data)==null?void 0:r.jobURL;return(e==null?void 0:e.length)>0&&e.toLowerCase().replace(/\/+$/,"").endsWith(`/${o}`)})||null)}).then(c=>{var o;return(o=c==null?void 0:c.data)!=null&&o.jobID&&(V=c.data.jobID,l.store("jobID",V),l.store("currentJobViewed",JSON.stringify(c))),c}).catch(()=>null),R}function j(s){typeof s!="object"&&console.warn("not object by objectChecker");for(let c in s)if(s[c]==="")return!1;return!0}function T(s){if(s=="apply"){i(".submitApplicationBtn, .applyText").css("pointer-events","none");return}i(".submitApplicationBtn, .applyText").css("pointer-events","auto"),i(".applyText").show()}function ne(){let s=N();ee().then(c=>{var o;(o=c==null?void 0:c.data)!=null&&o.screeningTemplateID&&l.submit({action:"Get Screening Questions",templateID:c.data.screeningTemplateID}).then(n=>{if(!n.status)return;let a=n.response.map(e=>pe(e));i(f).find("[data-rel=screening-fields]").empty().append(a)})})}function pe(s){var c,o;switch(s.questionType){case"Text":return`
                 <div class="input-field-container">
                    <label>
                        ${s.question}
                        <input type="text" autocomplete="nope" data-qtype="text" data-qid="${s.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Number":return`
                 <div class="input-field-container">
                    <label>
                        ${s.question}
                        <input type="number" autocomplete="nope" data-qtype="number" data-qid="${s.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Date":return`
                 <div class="input-field-container">
                    <label>
                        ${s.question}
                        <input type="date" autocomplete="nope" data-qtype="date" data-qid="${s.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Boolean":return`
                 <div class="input-field-container">
        ...
        `)),new Promise(e=>{new Promise((u,d)=>{if(t.inEditor&&!a){u(JSON.parse(o._mock));return}dmAPI.loadCollectionsAPI().then(b=>{b.data("Jobs").where("jobID","EQ",a).get().then(g=>{let p=g.values.length>0&&g.values[0].data.screeningTemplateID;p?(o._screeningTemplateID=p,b.data("Screening Questions").where("screeningTemplateID","EQ",p).get().then(m=>{u({response:{items:m.values.map(w=>w.data)}})},m=>{d(m)})):u({})})})}).then(u=>{if(!u||!u.response||!u.response.items||u.response.items.length==0){e();return}u.response.items.forEach(d=>{pIndex=Math.floor((parseInt(d.sortOrder)||0)/100),o._pages[pIndex]=o._pages[pIndex]||[],o._pages[pIndex].push(d)});for(let d=0;d<o._pages.length;d++)o._pages[d]||o._pages.splice(d,1);o._ko=u.response.items.filter(d=>d.knockOutDate||d.knockOutList||d.knockOutText||d.knockOutNumber||d.knockOutBoolean!==null),e()})})},this._showQuestions=(n,a=!0)=>{if(n>this.pageNumber&&!this._validate()){alert("Please answer all questions");return}this._recordAnswers().then(()=>{var d;n>this._maxPage&&(this._maxPage=n);let e=i(f).find("[data-rel=screening-fields]"),r=(this._pages[n]||[]).filter(b=>!b.parentQuestionID||o._answers[b.parentQuestionID]),u=r.map(b=>{var g;return o._questionElement({...b,options:(g=b.options)==null?void 0:g.filter(p=>{var m;return!b.parentQuestionID||((m=o._answers[b.parentQuestionID].answerUUID)==null?void 0:m.indexOf(p.parentOptionID))>=0})})});e.empty().append(u).append(o._pagingElements(n)),e.find(".sq-help-text[collapsible]").each(function(b,g){let p=i(g),m=i(p).outerHeight();p.find(".text-main").attr("style",`--shaz-help-text-lines: ${t.config.helpTextLines||3}`),p.outerHeight()<m&&p.addClass("collapsible").on("click","[data-rel=button-show-more]",function(){let w=i(this);p.hasClass("expanded")?(p.find(".text-main").css({display:"-webkit-box"}),p.removeClass("expanded")):(p.find(".text-main").css({display:"block"}),p.addClass("expanded"))})}),e.find("[data-rel=screening-page-index]").click(function(){let b=parseInt(i(this).attr("data-page"))||0;o._showQuestions(b)}),(d=this._pages[n])!=null&&d.find(b=>b.parentQuestionID)&&e.find("input, select").on("change",function(){i(this).is("[data-qtype=date]")||o._recordAnswers().then(()=>{o._showQuestions(n,!1)})}),e.find("[data-rel=screening-apply]").click(function(){if(!o._validate()){alert("Please answer all questions");return}l.pub(J.submit)}),a&&(e.get(0).scrollIntoView(),window.scrollBy({top:-200,behavior:"smooth"})),this._restoreAnswers(r.map(b=>b.screeningQuestionID)),this.pageNumber=n})},this._questionElement=n=>{var a,e,r,u,d,b,g,p,m,w,P,O,h,I;if(((a=n.parentQuestionID)==null?void 0:a.length)>0&&!(((e=n.options)==null?void 0:e.length)>0))return"";switch(n.questionType){case"Text":return`
                ...
                `;case"Number":return`
                     <div class="input-field-container">
                        ...
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="number" autocomplete="nope" data-qtype="number" data-qid="${n.screeningQuestionID}" ${n.isMandatory?"required":""} />
                        </label>
                        ${((u=n.helpText)==null?void 0:u.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                ...
                `;case"Date":return`
                     <div class="input-field-container">
                        ...
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="date" autocomplete="nope" data-qtype="date" data-qid="${n.screeningQuestionID}" ${n.isMandatory?"required":""} />

                        </label>
                        ${((d=n.helpText)==null?void 0:d.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}; --shaz-showMoreShadowColor: ${t.config.showMoreShadowColor}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                            </div>
                        </div>
                        `||""}
                     ...
                     <div class="input-field-container">
                        <label>
                            <p class="sq-boolean-question">
                                <input tabindex="30" type="checkbox" autocomplete="nope" data-qtype="bool" data-qid="${n.screeningQuestionID}" ${n.isMandatory?"required":""}  />
                                ${n.question}
                                ${n.isMandatory?"*":""}
                            </p>
                        </label>
                        ${((b=n.helpText)==null?void 0:b.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                ...
                `;case"List":{let Q=(g=n.options)==null?void 0:g.map(v=>`<option value="${v.screeningQuestionOptionsID}">${v.label||v.option}</option>`);return`
                     <div class="input-field-container">
                        <p class="sq-list-question">${n.question}</p>
                        <select data-qtype="list" data-qid="${n.screeningQuestionID}" ${n.isMandatory?"required":""}>${(Q==null?void 0:Q.join(""))||""}</select>
                        ${((p=n.helpText)==null?void 0:p.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                ...
                `}case"Multiselect Checkbox":{let Q=(m=n.options)==null?void 0:m.map(v=>`<label><input tabindex="30" type="checkbox" autocomplete="nope" data-qtype="check-list" data-qid="${n.screeningQuestionID}" data-value="${v.screeningQuestionOptionsID}" ${n.isMandatory?"required":""} />${v.label||v.option}</label>`);return`
                     <div class="input-field-container">
                        <p class="sq-checklist-question">
                            ${n.question}
                            ${n.isMandatory?"*":""}
                        </p>
                        ${((w=n.helpText)==null?void 0:w.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                        ...
                        `||""}
                        ${(Q==null?void 0:Q.join(""))||""}
                     </div>
                `}case"Radio":{let Q=(P=n.options)==null?void 0:P.map(v=>`<label class="sq-question-option"><input tabindex="30" type="radio" data-qtype="radio" name="${n.screeningQuestionID}" data-qid="${n.screeningQuestionID}" value="${v.screeningQuestionOptionsID}" ${n.isMandatory?"required":""} />${v.label||v.option}</label>`);return`
                     <div class="input-field-container">
                        <p class="sq-radio-question">
                            ${n.question}
                            ${n.isMandatory?"*":""}
                        </p>
                        ${((O=n.helpText)==null?void 0:O.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                        ...
                        `||""}
                        <div class="sq-opt-list">${(Q==null?void 0:Q.join(""))||""}</div>
                     </div>
                `}case"File":return`
                     <div class="input-field-container">
                        <label>
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="file" autocomplete="nope" data-qtype="file" data-qid="${n.screeningQuestionID}" ${n.isMandatory?"required":""} />
                        </label>
                        ${((h=n.helpText)==null?void 0:h.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                ...
                `;case"Header":return`
                    <div class='screening-question-heading'>
                        <h3>${n.question}</h3>
                        ${((I=n.helpText)==null?void 0:I.length)>0&&`
                        <div class="sq-help-text" ${n.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${n.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${t.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${t.config.showMoreHelpText}</a>
                ...
                `;default:return""}},this._pagingElements=n=>{let a=[];if(a.push(`<div data-rel="screening-pages" class="screening-pages-container" style="--shaz-text-align: ${t.config.PagingButtonAlignment}">`),n>0&&a.push(`<a href='javascript:void(0);' class="button-page-nav back" data-rel='screening-page-index' data-page='${n-1}'><span class="text">${t.config.pageBack||"Back"}</span></a>`),n<this._pages.length-1?a.push(`<a href='javascript:void(0);' class="button-page-nav forward" data-rel='screening-page-index' data-page='${n+1}'><span class="text">${t.config.pageNext||"Next"}</span></a>`):a.push(`<a href='javascript:void(0);' class="button-page-nav forward" data-rel='screening-apply'><span class="text">${t.config.applyTextBtn||"Apply"}</span></a>`),this._pages.length>1){let e=new Array(this._maxPage+1);for(let r=0;r<e.length;r++)e[r]=`<a href='javascript:void(0);' class="button-page-index ${r===n?"active":""}" data-rel='screening-page-index' data-page='${r}'></a>`;for(let r=e.length;r<this._pages.length;r++)e[r]=`<a href='javascript:void(0);' class="button-page-index disabled"></a>`;a.push(`<div class="sq-pages-index">${e.join("")}</div>`)}return a.push("</div>"),a.join("")},this._presentList=n=>{let a=n.map(r=>`
            <div>
                <input type="checkbox" data-value="${r.screeningQuestionOptionsID}" />
                ${r.option}
            </div>
        `),e=`
        ...
        `;i(e).appendTo(i(f))},this._recordAnswers=()=>Promise.all(i(f).find("[data-rel=screening-fields] input, [data-rel=screening-fields] select").map((n,a)=>{var r;let e=i(a);switch(e.attr("data-qtype")){case"text":return e.val()?o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerText:e.val(),screeningTemplateID:o._screeningTemplateID}:delete o._answers[e.attr("data-qid")],Promise.resolve();case"number":return e.val()?o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerNum:parseInt(e.val()),screeningTemplateID:o._screeningTemplateID}:delete o._answers[e.attr("data-qid")],Promise.resolve();case"date":return e.val()?o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerDate:e.val(),screeningTemplateID:o._screeningTemplateID}:delete o._answers[e.attr("data-qid")],Promise.resolve();case"bool":return o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerBoolean:e.is(":checked")?1:0,screeningTemplateID:o._screeningTemplateID},Promise.resolve();case"list":return e.val()?o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerUUID:[e.val()],screeningTemplateID:o._screeningTemplateID}:delete o._answers[e.attr("data-qid")],Promise.resolve();case"check-list":{let u=e.attr("data-qid"),d=((r=o._answers[u])==null?void 0:r.answerUUID)||[];return e.is(":checked")?d.indexOf(e.attr("data-value"))<0&&d.push(e.attr("data-value")):d.indexOf(e.attr("data-value"))>=0&&d.splice(d.indexOf(e.attr("data-value")),1),d.length===0?delete o._answers[e.attr("data-qid")]:o._answers[u]={screeningQuestionID:e.attr("data-qid"),answerUUID:d,screeningTemplateID:o._screeningTemplateID},Promise.resolve()}case"radio":return e.is(":checked")&&(o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerUUID:[e.attr("value")],screeningTemplateID:o._screeningTemplateID}),Promise.resolve();case"file":{let u=e.get(0).files.length>0&&e.get(0).files[0];return u?o._readFile(u).then(d=>(o._answers[e.attr("data-qid")]={screeningQuestionID:e.attr("data-qid"),answerFile:d,answerFileName:u.name,screeningTemplateID:o._screeningTemplateID},Promise.resolve())):(delete o._answers[e.attr("data-qid")],Promise.resolve())}default:return Promise.resolve()}})),this._restoreAnswers=n=>{for(let a in o._answers){let e=o._answers[a];if(a=a.split(":")[0],!(n.indexOf(a)<0)){if(i(f).find(`[data-rel=screening-fields] input[data-qid=${a}]:visible, [data-rel=screening-fields] select[data-qid=${a}]:visible`).length===0){delete o._answers[a];continue}e.answerText||e.answerNum||e.answerDate?i(f).find(`[data-rel=screening-fields] input[data-qid=${a}]`).val(e.answerText||e.answerNum||e.answerDate):e.answerBoolean&&!e.answerUUID?i(f).find(`[data-rel=screening-fields] input[data-qid=${a}]`).attr("checked",!0):e.answerUUID?(typeof e.answerUUID=="string"&&(e.answerUUID=e.answerUUID.split(",")),e.answerUUID.forEach(r=>{i(f).find(`[data-rel=screening-fields] input[data-qid=${a}][data-value=${r.trim()}]`).attr("checked",!0),i(f).find(`[data-rel=screening-fields] input[data-qid=${a}][value=${r.trim()}]`).attr("checked",!0),i(f).find(`[data-rel=screening-fields] select[data-qid=${a}]`).val(r.trim())})):e.answerFile&&i(f).find(`[data-rel=screening-fields] input[data-qid=${a}]`).val(this._fileBlob(e.answerFile))}}},this._validate=()=>{let n=!0;return i(f).find("[data-rel=screening-fields] input[required], [data-rel=screening-fields] select[required]").each((a,e)=>{let r=i(e);switch(r.attr("data-qtype")){case"text":case"select":{let u=r.val();n=n&&u&&u.trim().length>0;break}case"bool":{n=n&&r.is(":checked");break}case"number":{let u=r.val();n=n&&u&&u.trim().length>0&&!isNaN(u);break}case"date":{let u=Date.parse(r.val());n=n&&!isNaN(u);break}case"radio":{n=n&&i(f).find(`[data-rel=screening-fields] input[type=radio][name=${r.attr("name")}]`).is(":checked");break}case"file":{n=n&&r.get(0).files.length>0;break}case"check-list":n=n&&i(f).find(`[data-rel=screening-fields] input[data-qtype=check-list][data-qid=${r.attr("data-qid")}]`).is(":checked")}}),n},this._readFile=n=>new Promise(a=>{let e=new FileReader;e.addEventListener("load",()=>{a(btoa(e.result))},!1),e.readAsBinaryString(n)}),this._fileBlob=(n,a="",e=512)=>{let r=atob(n),u=[];for(let b=0;b<r.length;b+=e){let g=r.slice(b,b+e),p=new Array(g.length);for(let w=0;w<g.length;w++)p[w]=g.charCodeAt(w);let m=new Uint8Array(p);u.push(m)}return new Blob(u,{type:a})},this._showEditor=()=>{let n=e=>{var r,u;return e.knockOutDate?new Date(e.knockOutDate).toLocaleDateString():e.knockOutList?(u=(r=e.options)==null?void 0:r.find(d=>d.screeningQuestionOptionsID===e.knockOutList))==null?void 0:u.option:e.knockOutText?e.knockOutText:e.knockOutNumber?e.knockOutNumber:e.knockOutBoolean!==null?e.knockOutBoolean?"Yes":"No":""},a=e=>{var r,u;return`
            <article data-rel="knockout" data-id="${e.screeningQuestionID}">
                ...
                <div class="row">
                    <label>
                        Enable
                        <input type="checkbox" data-rel="enable" ${((r=e.alert)==null?void 0:r.length)>0||((u=e.redirect)==null?void 0:u.length)>0?'checked="checked"':""} />
                    </label>
                </div>
            </article>
        `};o._ko.length!==0&&s.config().then(e=>{let r=i(f).find("[data-dialog=knockout]");r.find("[data-rel=dialog-content]").empty().append(o._ko.map(d=>{var b;return a({...d,...(b=e==null?void 0:e.knockout)==null?void 0:b.find(g=>g.screeningQuestionID===d.screeningQuestionID)})})),i(f).find("[data-rel=screening-fields] .button-editor").remove(),i("<button />").text("Edit Knockout Handling").addClass("button-editor").on("click",function(){r.toggle()}).prependTo(i(f).find("[data-rel=screening-fields]"));let u=d=>{let b=i(d.target);i("<input />").attr({type:"checkbox","data-rel":"enable",checked:b.is(":checked")?"checked":null}).on("click",u).appendTo(b.parent()),b.remove()};r.find("[data-rel=enable]").on("click",u),r.find("[data-rel=button-submit]").on("click",function(){let d=(e==null?void 0:e.knockout)||[];r.find("[data-rel=dialog-content] article [data-rel=enable]").each((b,g)=>{let p=i(g).parents("article"),m=d.find(w=>w.screeningQuestionID===p.attr("data-id"));m&&d.splice(d.indexOf(m),1),i(g).is(":checked")&&d.push({screeningQuestionID:p.attr("data-id"),alert:p.find("[data-rel=alert]").val(),redirect:p.find("[data-rel=redirect]").val()})}),s.config().then(b=>s.config({...b,knockout:d})).then(()=>{e.knockout=d}),r.toggle()}),r.find("[data-rel=button-dismiss]").on("click",function(){r.toggle(),r.find("[data-rel=dialog-content]").empty().append(o._ko.map(d=>{var b;return a({...d,...(b=e==null?void 0:e.knockout)==null?void 0:b.find(g=>g.screeningQuestionID===d.screeningQuestionID)})})),r.find("[data-rel=enable]").on("click",u)})})},this._fetchAnswers=n=>l.site().then(a=>l.submit({action:"Get Screening Answers",siteID:a.siteID,jobApplicationID:n})).then(a=>{var r,u;let e=[];return(u=(r=a==null?void 0:a.response)==null?void 0:r.items)==null||u.forEach(d=>{e[d.screeningQuestionID]=d}),Promise.resolve(e)}),this._fetchQuestions().then(()=>{this._showQuestions(0,!1),t.inEditor&&this._showEditor()}),c&&this._fetchAnswers(c).then(n=>{let a=this._pages[0]||[];this._answers=n,this._restoreAnswers(a.map(e=>e.screeningQuestionID))}),this}let De=s=>{let c=Ie(s,new URL(window.location).searchParams.get("jobApplicationID")),o=()=>{let e=n(),r=j(e),u=F;return r?k&&!e.cVFileContent?(alert(t.config.warningResume||"Please upload your resume"),Promise.reject()):u?l.firebase().validateEmail(e.eMail).then(()=>(e.candidateID=l.uuid(),t.config.showPassword||!t.config.useAnonymous?l.firebase().create(e.eMail,e.password).then(d=>Promise.resolve({...e,firebaseUserID:d.uid})).catch(d=>(T("hide"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),d.code==="auth/invalid-email"?alert(t.config.warningEmail||"Please use a valid email address"):d.code=="auth/email-already-in-use"&&t.config.showPassword&&window.confirm("The email is already an existing account please login instead.")&&(window.scrollTo({top:0,behavior:"smooth"}),i(f).find(".submitApplicationBtn").css("pointer-events","auto"),i(f).find(".applyText").show(),i(f).find(".fcLoader").removeClass("fcLoadingSeek")),Promise.reject(d))):Promise.resolve({...e,isValidated:!1}))).catch(()=>(T("hide"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),alert(t.config.warningEmail||"Please supply a valid email."),Promise.reject())):(T("hide"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),alert(t.config.warningPassword||"Please follow the password pattern!"),Promise.reject()):(T("hide"),alert(t.config.warningFields||"Please complete all required fields"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject())},n=()=>{var d,b,g,p,m,w,P;let e=i("#passwordInput").val()||"",r=i(f);t.config.showPassword||(e=l.uuid(),F=!0);let u={salutation:i(f).find("#salutation").val()||" ",firstName:((b=(d=r.find("#firstName").val())==null?void 0:d.trim())==null?void 0:b.replace(/["«»‘’‚‛“”„‟‹›❛❜❝❞❮❯〝〞〟＂❟❠⹂🙶🙷🙸＇]/g,""))||"",surname:((p=(g=r.find("#lastName").val())==null?void 0:g.trim())==null?void 0:p.replace(/["«»‘’‚‛“”„‟‹›❛❜❝❞❮❯〝〞〟＂❟❠⹂🙶🙷🙸＇]/g,""))||"",eMail:((m=r.find("#emailAddress").val())==null?void 0:m.trim())||"",phone:t.config.showPhoneNumber&&((w=r.find("#phoneNumber").val())==null?void 0:w.trim())||(t.config.requirePhone?"":void 0),mobile:t.config.showMobileNumber&&((P=r.find("#mobileNumber").val())==null?void 0:P.trim())||(t.config.requireMobile?"":void 0),password:e,isActive:!0,isValidated:!0,isSubscribed:W&&r.find("[data-rel=checkbox-subscribe]:checked").length>0,dudaSiteID:t.siteId,cVFileContent:S.resumeFile&&btoa(S.resumeFile),cVFileName:S.resumeName};return s&&t.config.enableTracing&&(!S.resumeFile||!S.coverLetterFile)&&(s.warn("file(s) missing",u),s.warn("file(s) missing (contains uploads)",S)),t.config.validatePhoneNumber&&(u.defaultPhoneCountry=t.config.defaultPhoneCountry||"AU",u.validatePhone=!0),u};s.sub(J.submit,()=>{if(!N()){alert(t.config.warningNoJob||"No job selected!");return}let e=document.querySelector("input#resume[type=file]"),r=document.querySelector("input#cover[type=file]"),u=k||A||Object.keys(S).length||i(f).find("#uploadExisting").is(":checked");if(i(f).find(".fcLoader").addClass("fcLoadingSeek"),l.store("previousApplicationPage",null),!c.isValid){T("hide"),alert(t.config.warningQuestions||"Please complete all of the screening questions"),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}if(t.config.requireApproval===!0&&!i(f).find("[data-rel=field-approve]").is(":checked")){T("hide"),alert(t.config.approvalWarning||"Please confirm approval of the terms"),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}let d=(g,p)=>(p==null?void 0:p.length)>0&&p||(g==null?void 0:g.length)>0&&g||"",b=i(f).find("input.invalid").map((g,p)=>`- ${d(i(p).attr("data-name"),i(p).attr("placeholder"))}`).toArray();if(b.length>0){T("hide"),alert(`${t.config.warningValidation||"Please correct the following:"}
${b.join(`
`)}`),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}c.answers().then(g=>{let p=c.knockout(g);if((p==null?void 0:p.length)>0){s.config().then(O=>{p.forEach(h=>{var Q,v,x;let I=(Q=O==null?void 0:O.knockout)==null?void 0:Q.find(C=>C.screeningQuestionID===h.screeningQuestionID);if(((v=I==null?void 0:I.alert)==null?void 0:v.length)>0&&alert(I.alert),((x=I==null?void 0:I.redirect)==null?void 0:x.length)>0){let C=t.inEditor?`/site/${E}${I.redirect}?preview=true&insitepreview=true&dm_device=desktop`:I.redirect;window.location=C}})}),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}let m=O=>{var I,Q;let h=n();return j(h)?k&&!h.cVFileContent?(T("hide"),alert(t.config.warningResume||"Please upload your resume"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject()):(h.cVFileContent=(I=h.cVFileContent)!=null?I:O.candidate.cVFileContent,h.cVFileName=(Q=h.cVFileName)!=null?Q:O.candidate.cVFileName,h.phone=h.phone||O.candidate.phone||null,h.mobile=h.mobile||O.candidate.mobile||null,delete h.dudaSiteID,delete h.defaultPhoneCountry,delete h.validatePhone,l.submit({action:"Edit Candidate Info",...O.candidate,...h}).then(v=>v.status&&l.user(!0)||Promise.reject())):(T("hide"),alert(t.config.warningFields||"Please complete all required fields"),i(f).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject())},w=O=>{var te,se,oe,ae;let h={referralSource:_.searchParams.get("utm_source")||l.session("referralSource"),referralMedium:_.searchParams.get("utm_medium")||l.session("referralMedium"),referralTerm:_.searchParams.get("utm_term")||l.session("referralTerm"),referralCampaign:_.searchParams.get("utm_campaign")||l.session("referralCampaign"),referralContent:_.searchParams.get("utm_content")||l.session("referralContent")},I={jobID:N(),screeningAnswers:g,...h};if(u){if(i("#uploadExisting").is(":checked")&&((te=O==null?void 0:O.candidate)!=null&&te.cVFileContent))I.resumeFile=(se=O==null?void 0:O.candidate)==null?void 0:se.cVFileContent,I.resumeFileName=(oe=O==null?void 0:O.candidate)==null?void 0:oe.cVFileName;else try{let y=Y(e,k,"resume");I.resumeFile=y&&btoa(S.resumeFile),I.resumeFileName=y&&S.resumeName,I.resumeUpdatedOn=y&&new Date}catch(y){i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}try{let y=Y(r,A,"coverLetter");I.coverLetterFile=y&&btoa(S.coverLetterFile),I.coverLetterFileName=y&&S.coverLetterFileName,I.coverLetterUpdatedOn=y&&new Date}catch(y){i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}if(e&&k&&!I.resumeFile){alert(t.config.warningResume||"Please upload your resume");return}if(r&&A&&!I.coverLetterFile){alert(t.config.warningCoverLetter||`Please upload your cover letter, or tick the "I don't have cover letter"`);return}}t.config.showGender&&(I.customField1=i("#gender").val()),t.config.showAboriginal&&(I.customField2=i("#aboriginal").is(":checked")),t.config.enableTracing&&(I.session=l.uuid(),s.log("apply",{candidate:O,application:I}));let Q;try{Q=JSON.parse(l.store("currentJobViewed"))}catch(y){}let v=(Q==null?void 0:Q.data)||{},x,C=[v[t.config.brochureField||"customField2"],v.applicationURL].find(y=>typeof y=="string"&&y.length>0);if((C==null?void 0:C.length)>0)x=C;else if(x=`/${L}`,t.config.includeLastSearch&&(Q!=null&&Q.data)){let y=[];(t.config.redirectJobField||[]).forEach(re=>{let B=Q.data[re.field];(B==null?void 0:B.length)>0&&y.push(`${re.field}=${encodeURIComponent(B)}`)}),y.length&&(x=`/${L}?${y.join("&")}`)}let ie={action:"Apply Job",dudaSiteID:t.siteId,application:I,candidate:O};if(I.resumeFile||I.coverLetterFile){l.submit(ie).then(()=>{T("stop"),M.clearKeys(),window.location.href=x}).catch(()=>{T("hide"),alert(t.config.warningApply||"Sorry, your application could not be submitted. Please try again."),i(f).find(".fcLoader").removeClass("fcLoadingSeek")});return}let Qe=((ae=l._site)==null?void 0:ae.RegionalUrl)||"https://shazamme.io/Job-Listing/src/php/regional/actions";try{fetch(Qe,{method:"POST",keepalive:!0,headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:JSON.stringify(ie)})}catch(y){}T("stop"),M.clearKeys(),window.location.href=x},P=n();if(!j(P)){T("hide"),alert(t.config.warningFields||"Please complete all required fields"),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}if(k&&!P.cVFileContent){T("hide"),alert(t.config.warningResume||"Please upload your resume"),i(f).find(".fcLoader").removeClass("fcLoadingSeek");return}P.candidateID=l.uuid(),w(P)})}),i(f).find(".eye").on("click",function(){i(this).toggleClass("fa-eye-slash").toggleClass("fa-eye").siblings("input").attr("type",function(r,u){return u==="text"?"password":"text"})}),t.config.validatePhoneNumber&&typeof libphonenumber=="object"&&i(f).find("input[type=telephone]").on("change",function(){let e=i(this);if(e.removeClass("invalid"),e.val().trim().length!==0)try{let r=libphonenumber.parsePhoneNumber(e.val(),t.config.defaultPhoneCountry||"AU");!r.country||!r.isValid()?e.addClass("invalid"):e.val(r.formatInternational())}catch(r){e.addClass("invalid")}}),l.store("applicationURL",window.location.href),ee().then(e=>{e!=null&&e.data?e.data.jobID===t.config.jobID&&s.pub("job-application-fixed-job",e.data.jobID,!0):t.inEditor||(i(".shmApplicationMainContainer").hide(),i(".section-no-job-message").show())});let a=e=>{if(e!=null&&e.isNew){K({salutation:" ",firstName:e.firstName,surname:e.lastName,eMail:e.email,phone:"",mobile:"",isNew:!0});return}let r=e==null?void 0:e.candidate;K(r),r!=null&&r.candidateID&&ge(r.candidateID).then(u=>{if(u)if(confirm(t.config.duplicateWarning||"DUPLICATE WARNING MESSAGE")){let d=window.location.href.includes(E)?`/site/${E}/${Z}?preview=true&insitepreview=true&dm_device=desktop`:`/${Z}`;window.location.href=d}else window.location.reload()})};l.user().then(e=>{a(e)}),s.sub("site-auth",e=>{a(e)})};i("head").append(i('<link rel="stylesheet" type="text/css" href="https://sdk.shazamme.io/css/fontawesome/css/fontawesome.min.css" crossorigin="anonymous" />')).append(i('<link rel="stylesheet" type="text/css" href="https://sdk.shazamme.io/css/fontawesome/css/regular.min.css" crossorigin="anonymous" />')),window.__shazLoadScript("https://sdk.shazamme.io/js/shazamme-1.0.3.min.js").then(function(){Promise.all([l.ready(t.siteId,t.page),l.script("https://sdk.shazamme.io/js/plugin/libphonenumber/1.10.54/plugin.min.js")]).then(()=>{De(l.register("application-screening-form",t,!0))})})}return we(Ce);})();
(function(){
