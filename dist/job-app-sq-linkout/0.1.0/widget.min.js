 ...
 * Build cd81c273baaf. Registers window.ShazammeWidget["<name>"].
 */
var __shazWidgetExport=(()=>{var Y=Object.defineProperty;var Te=Object.getOwnPropertyDescriptor;var Se=Object.getOwnPropertyNames;var ke=Object.prototype.hasOwnProperty;var Pe=(Q,s)=>{for(var u in s)Y(Q,u,{get:s[u],enumerable:!0})},xe=(Q,s,u,i)=>{if(s&&typeof s=="object"||typeof s=="function")for(let b of Se(s))!ke.call(Q,b)&&b!==u&&Y(Q,b,{get:()=>s[b],enumerable:!(i=Te(s,b))||i.enumerable});return Q};var Ce=Q=>xe(Y({},"__esModule",{value:!0}),Q);var _e={};Pe(_e,{default:()=>De});var Ee="https://sdk.shazamme.io/js/shazamme-1.0.3.min.js",Le=/shazamme(-1\.0\.\d+(-test)?)?\.min\.js/;function ge(Q){return new Promise((s,u)=>{let i=document.createElement("script");i.src=Q,i.onload=()=>s(),i.onerror=()=>u(new Error(`Failed to load ${Q}`)),(document.head||document.documentElement).appendChild(i)})}function $e(){if(window.__shazSDKPromise)return window.__shazSDKPromise;let Q=window.shazamme?Promise.resolve():ge(Ee);return window.__shazSDKPromise=Q,Q}function Me(Q){let s=window.__shazScriptCache=window.__shazScriptCache||{},u=s[Q];if(u)return u;let i=ge(Q).catch(b=>{throw delete s[Q],b});return s[Q]=i,i}function Ae(){typeof window=="undefined"||window.__shazLoadScript||(window.__shazLoadScript=Q=>Le.test(Q)?$e():Me(Q))}function Ie(){Ae()}function De(Q){Ie();var s=Q.data,u=Q.element,i=Q.$||window.jQuery||window.$,b=Q.shazamme||window.shazamme;let M=new Qe,T={},Fe=!1,Ne=!1,C=s.siteId,He={},E="thank-you",K="dashboard",qe="register",j;s.config.toggleThankyouPage&&(E=s.config.txt_Thankyoupageurl);let P=s.config.showResume===!0,A=s.config.applicationLayout!=="simpleForm"&&s.config.showCoverLetter,X=s.config.showSubscription==="true",_=new URL(window.location.href),F=!1,$=document.getElementById("passwordInput"),ee={submit:"application-screening-form-submit"},H=(parseInt(s.config.maxUploadSize)||10)*1024*1024;if(s.config.showScreeningQuestions&&ce(),i("#passwordInput").focus(function(){i("#passwordMessage,.arrow-bottom").css("display","flex")}),i("#passwordInput").blur(function(){F&&i("#passwordMessage, .arrow-bottom").css("display","none")}),s.config.useSmartLogin&&!Oe()){var q=null;i(u).find(".smart-login-login, .smart-login-register").hide(),i("#emailLogin").keyup(function(){q&&clearTimeout(q),q=setTimeout(()=>{let t=i(this).val().trim();if(t.length===0){i(u).find(".smart-login-login, .smart-login-register").hide();return}firebase.auth().fetchSignInMethodsForEmail(t).then(c=>{i(u).find(".smart-login-login, .smart-login-register").hide(),c.length>0?i(u).find(".smart-login-login").show():(i(u).find(".smart-login-register").show(),i("#emailAddress").val(t))}).catch(()=>{i(u).find(".smart-login-login").hide(),i(u).find(".smart-login-register").show()}),q=null},500)})}if($){let t=document.getElementById("capital"),c=document.getElementById("number"),a=document.getElementById("length"),e=document.getElementById("specialChar");$.onkeyup=function(){let r=0;if($.value.length<8?(a.classList.remove("valid"),a.classList.add("invalid"),r-=1):(a.classList.remove("invalid"),a.classList.add("valid"),r+=1),$.value.search(/(?=.*[A-Z])/)!=-1?(t.classList.remove("invalid"),t.classList.add("valid"),status=!0,r+=1):(t.classList.remove("valid"),t.classList.add("invalid"),r-=1),$.value.search(/\d/)!=-1?(c.classList.remove("invalid"),c.classList.add("valid"),r+=1):(c.classList.remove("valid"),c.classList.add("invalid"),r-=1),$.value.search(/(?=.*[!@#$%^&*])/)!=-1?(e.classList.remove("invalid"),e.classList.add("valid"),r+=1):(e.classList.remove("valid"),e.classList.add("invalid"),r-=1),r===4){i(".passwordErrorTitle").text("Your password is acceptable!"),i("#passwordMessage").css("background-color","#59db5d"),i(".valid,.passwordErrorTitle").css("color","#fff"),i(".arrow-bottom").css({"border-top":"15px solid #59db5d"}),F=!0;return}else return i(".passwordErrorTitle").text("Password must contain the following"),i("#passwordMessage").css("background-color","#ffa1a1"),i("#passwordMessage,.passwordErrorTitle, .invalid").css("color","red"),i(".arrow-bottom").css({"border-top":"15px solid #ffa1a1"}),F=!1,!1}}function U(t,c){var a=t.files[0];let e=a.name.replace(/[^a-z0-9-_.]/gi,"-").replace(/-{2,}/gi,"-");var r=new FileReader;let n=c=="resume"?"resumeFile":"coverLetterFile",o=c=="resume"?"resumeName":"coverLetterFileName";r.addEventListener("load",function(){T[n]=r.result,T[o]=e},!1),a&&r.readAsBinaryString(a)}i(u).find("input#resume").on("change",function(){var e;let t=this;if(((e=t.files[0])==null?void 0:e.size)>H){alert(s.config.resumeSizeWarning||`Error! File too large, Should not exceed ${s.config.maxUploadSize||""}mb`),i(t).val("");return}let a=i("input[type=radio][name=uploadTypeResume][value=uploadResume]");a.get(0).checked=!0,a.trigger("change"),U(t,"resume")}).each(function(){var e;let t=this,c=(e=t.files[0])==null?void 0:e.size;if(t.files.length==0)return;if(c>H){alert(s.config.resumeSizeWarning||`Error! File too large, Should not exceed ${s.config.maxUploadSize||""}mb`),i(t).val("");return}let a=i("input[type=radio][name=uploadTypeResume][value=uploadResume]");a.get(0).checked=!0,a.trigger("change"),U(t,"resume")}),i(u).find("input#cover").on("change",function(){var e;let t=this;if(((e=t.files[0])==null?void 0:e.size)>H){alert(s.config.resumeSizeWarning||`Error! File too large, Should not exceed ${s.config.maxUploadSize||""}mb`),i(t).val("");return}let a=i("input[type=radio][name=uploadTypeCover][value=uploadCover]");a.get(0).checked=!0,a.trigger("change"),U(t,"cover")}).each(function(){var e;let t=this,c=(e=t.files[0])==null?void 0:e.size;if(t.files.length==0)return;if(c>H){alert(s.config.resumeSizeWarning||`Error! File too large, Should not exceed ${s.config.maxUploadSize||""}mb`),i(t).val("");return}let a=i("input[type=radio][name=uploadTypeCover][value=uploadCover]");a.get(0).checked=!0,a.trigger("change"),U(t,"cover")}),i("input[type=radio][name=uploadTypeResume]").change(function(){this.value=="uploadResume"?(i(".fileUploadContainer").css("display","flex"),P=!0):(i(".fileUploadContainer").css("display","none"),P=!1)}),i("input[type=radio][name=uploadTypeCover]").on("change",function(){this.value=="uploadCover"?(i(".coverUploadContainer").css("display","flex"),A=!0):(i(".coverUploadContainer").css("display","none"),A=!1)}).first().attr("checked","checked").trigger("change"),i("#loginBtn").click(function(){let t=i("#emailLogin").val(),c=i("#passwordLogin").val();j="loginForm",M.nativeLogin(t,c)}),i(".container-item").click(function(){switch(i(".registerTxt").hide(),i(this).attr("data-provider")){case"facebook":{M.loginOrRegisterUsingProvider(facebookProvider);break}case"google":{M.loginOrRegisterUsingProvider(googleProvider);break}case"linkedin":{b.oauth("linkedinProvider");break}case"seek":{b.oauth("seekProvider");break}}});function ne(t,c,a){if(c){let e=a=="resume"?s.config.warningResume||"Please upload your resume":s.config.warningCoverLetter||`Please upload your cover letter, or tick the "I don't have cover letter"`;if(t.files[0]==null){throw y("stop"),i(t).find(".fcLoader").removeClass("fcLoadingSeek"),alert(e),new Error(e);return null}return t.files[0]}return null}function Ue(t,c){t&&(T[c]=t)}function Qe(){this.clearKeys=()=>{let t=["seekAuthorizationCode","authProvider","currentJobViewed","linkedIncode","jobID"];for(let c of t)b.store(c,null)},this.nativeLogin=(t,c)=>{b.firebase().auth(t,c).then(()=>{if(s.inEditor)return;if(j=="loginForm"){window.location.reload();return}if(j=="registrationForm"){let e=window.location.href.includes(C)?`/site/${C}/${E}?preview=true&insitepreview=true&dm_device=desktop`:`/${E}`;window.location.href=e;return}let a=b.store("previousApplicationPage");if(a){window.location.href=a;return}window.location.href=window.location.href.includes(C)?`/site/${C}/${E}?preview=true&insitepreview=true&dm_device=desktop`:`/${E}`}).catch(a=>{alert(a.msg)})},this.loginOrRegisterUsingProvider=t=>{b.firebase().oauth(t).then(c=>{c.isNew?b.submit({action:"Register Candidate",salutation:" ",firstName:c.firstName,surname:c.lastName,eMail:c.email,password:b.uuid(),candidateID:b.uuid(),firebaseUserID:c.firebaseUserID,isActive:!0,isValidated:!0,isSubscribed:X&&i(u).find("[data-rel=checkbox-subscribe]:checked").length>0,dudaSiteID:s.siteId}).then(a=>{a.status&&!s.inEditor&&b.auth(c.email,c.firebaseUserID,!0).then(()=>{window.location.reload()})}):window.location.reload()}).catch(c=>{alert(c.msg)})}}function ie(t){let c=i(u);t?(c.find("#salutation").val(t.salutation||""),c.find("#firstName").val(t.firstName||""),c.find("#lastName").val(t.surname||""),c.find("#emailAddress").val(t.eMail||"").attr("readonly","readonly"),c.find("#phoneNumber").val(t.phone||""),c.find("#mobileNumber").val(t.mobile||""),t.isNew?(c.find(".shmLoginContainer,.applyMainSocial,.subscriptionContainer").show(),c.find(".greeting-message").hide()):(c.find(".shmLoginContainer,.applyMainSocial,.subscriptionContainer").hide(),c.find(".greeting-message").show().text(`${s.config.greeting||"Welcome back"} ${t.firstName||""} ${t.surname||""}!`))):(c.find(".shmregistrationContainer,.shmLoginContainer,.applyMainSocial,.subscriptionContainer").show(),c.find(".greeting-message").hide().text(""),c.find("#salutation").val(""),c.find("#firstName").val(""),c.find("#lastName").val(""),c.find("#emailAddress").val("").removeAttr("readonly"),c.find("#phoneNumber").val(""),c.find("#mobileNumber").val("")),s.inEditor&&c.find(".greeting-message").show().text(`${s.config.greeting||"Welcome back"}!`),t!=null&&t.cVFileContent||i(u).find(".uploadExisting").css("display","none"),s.config.showScreeningQuestions&&ce()}function Oe(){return window.localStorage.vinylResponse}let te=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i,z=null,B=null;function Z(){let t=new URL(window.location.href);for(let[c,a]of t.searchParams.entries())if(c.toLowerCase()==="jobid"&&(a==null?void 0:a.length)>0)return a;return s.config.useSingleJob&&s.config.jobID||b.store("jobID")}function N(){return z||Z()}function se(){let t=Z();return(t==null?void 0:t.length)>0&&!te.test(t)?String(t).trim().replace(/\/+$/,"").split("/").pop():null}function oe(){let t=se();if(!t)return null;let c=String(s.config.BrochurePagePath||"/dynamic-brochure").replace(/^\/+|\/+$/g,"");return s.inEditor?`/site/${s.siteId}/${c}/${t}?preview=true&insitepreview=true&dm_device=desktop`:`https://${window.location.hostname}/${c}/${t}`}function ae(){return s.config.showBrochureLink===!0||s.config.showBrochureLink==="true"}function W(){let t=s.config.screeningTemplateId||s.config.screeningTemplateID||new URL(window.location.href).searchParams.get("screeningTemplateId");return typeof t=="string"&&t.trim().length>0?t.trim():null}function re(){if(B)return B;let t=Z();return B=b.site().then(c=>{if(!((t==null?void 0:t.length)>0))return null;if(te.test(t))return b.fetch({path:`/job-results/${c.siteID}/${t}`,isExternal:!0,useCache:!0});let a=String(t).toLowerCase().replace(/\/+$/,"").split("/").pop();return b.fetch({path:`/job-results/${c.siteID}`,isExternal:!0,useCache:!0}).then(e=>(e||[]).find(r=>{var o;let n=(o=r==null?void 0:r.data)==null?void 0:o.jobURL;return(n==null?void 0:n.length)>0&&n.toLowerCase().replace(/\/+$/,"").endsWith(`/${a}`)})||null)}).then(c=>{var a;return(a=c==null?void 0:c.data)!=null&&a.jobID&&(z=c.data.jobID,b.store("jobID",z),b.store("currentJobViewed",JSON.stringify(c))),c}).catch(()=>null),B}function J(t){typeof t!="object"&&console.warn("not object by objectChecker");for(let c in t)if(t[c]==="")return!1;return!0}function y(t){if(t=="apply"){i(".submitApplicationBtn, .applyText").css("pointer-events","none");return}i(".submitApplicationBtn, .applyText").css("pointer-events","auto"),i(".applyText").show()}function ce(){let t=N();re().then(c=>{var e;let a=W()||((e=c==null?void 0:c.data)==null?void 0:e.screeningTemplateID);a&&b.submit({action:"Get Screening Questions",templateID:a}).then(r=>{if(!r.status)return;let n=r.response.map(o=>he(o));i(u).find("[data-rel=screening-fields]").empty().append(n)})})}function he(t){var c,a;switch(t.questionType){case"Text":return`
                 <div class="input-field-container">
                    <label>
                        ${t.question}
                        <input type="text" autocomplete="nope" data-qtype="text" data-qid="${t.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Number":return`
                 <div class="input-field-container">
                    <label>
                        ${t.question}
                        <input type="number" autocomplete="nope" data-qtype="number" data-qid="${t.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Date":return`
                 <div class="input-field-container">
                    <label>
                        ${t.question}
                        <input type="date" autocomplete="nope" data-qtype="date" data-qid="${t.screeningQuestionID}" />
                    </label>
                </div>
            `;case"Boolean":return`
                 <div class="input-field-container">
        ...
        `)),new Promise(n=>{new Promise((d,f)=>{if(s.inEditor&&!r){d(JSON.parse(a._mock));return}dmAPI.loadCollectionsAPI().then(l=>{let D=W();if(D){a._screeningTemplateID=D,l.data("Screening Questions").where("screeningTemplateID","EQ",D).get().then(g=>d({response:{items:g.values.map(I=>I.data)}}),g=>f(g));return}l.data("Jobs").where("jobID","EQ",r).get().then(g=>{let I=g.values.length>0&&g.values[0].data.screeningTemplateID;I?(a._screeningTemplateID=I,l.data("Screening Questions").where("screeningTemplateID","EQ",I).get().then(v=>{d({response:{items:v.values.map(k=>k.data)}})},v=>{f(v)})):d({})})})}).then(d=>{if(!d||!d.response||!d.response.items||d.response.items.length==0){n();return}d.response.items.forEach(f=>{pIndex=Math.floor((parseInt(f.sortOrder)||0)/100),a._pages[pIndex]=a._pages[pIndex]||[],a._pages[pIndex].push(f)});for(let f=0;f<a._pages.length;f++)a._pages[f]||a._pages.splice(f,1);a._ko=d.response.items.filter(f=>f.knockOutDate||f.knockOutList||f.knockOutText||f.knockOutNumber||f.knockOutBoolean!==null),n()})})},this._showQuestions=(e,r=!0)=>{if(e>this.pageNumber&&!this._validate()){alert("Please answer all questions");return}this._recordAnswers().then(()=>{var f;e>this._maxPage&&(this._maxPage=e);let n=i(u).find("[data-rel=screening-fields]"),o=(this._pages[e]||[]).filter(l=>!l.parentQuestionID||a._answers[l.parentQuestionID]),d=o.map(l=>{var D;return a._questionElement({...l,options:(D=l.options)==null?void 0:D.filter(g=>{var I;return!l.parentQuestionID||((I=a._answers[l.parentQuestionID].answerUUID)==null?void 0:I.indexOf(g.parentOptionID))>=0})})});n.empty().append(d).append(a._pagingElements(e)),n.find(".sq-help-text[collapsible]").each(function(l,D){let g=i(D),I=i(g).outerHeight();g.find(".text-main").attr("style",`--shaz-help-text-lines: ${s.config.helpTextLines||3}`),g.outerHeight()<I&&g.addClass("collapsible").on("click","[data-rel=button-show-more]",function(){let v=i(this);g.hasClass("expanded")?(g.find(".text-main").css({display:"-webkit-box"}),g.removeClass("expanded")):(g.find(".text-main").css({display:"block"}),g.addClass("expanded"))})}),n.find("[data-rel=screening-page-index]").click(function(){let l=parseInt(i(this).attr("data-page"))||0;a._showQuestions(l)}),(f=this._pages[e])!=null&&f.find(l=>l.parentQuestionID)&&n.find("input, select").on("change",function(){i(this).is("[data-qtype=date]")||a._recordAnswers().then(()=>{a._showQuestions(e,!1)})}),n.find("[data-rel=screening-apply]").click(function(){if(!a._validate()){alert("Please answer all questions");return}b.pub(ee.submit)}),r&&(n.get(0).scrollIntoView(),window.scrollBy({top:-200,behavior:"smooth"})),this._restoreAnswers(o.map(l=>l.screeningQuestionID)),this.pageNumber=e})},this._questionElement=e=>{var r,n,o,d,f,l,D,g,I,v,k,S,h,m;if(((r=e.parentQuestionID)==null?void 0:r.length)>0&&!(((n=e.options)==null?void 0:n.length)>0))return"";switch(e.questionType){case"Text":return`
                ...
                `;case"Number":return`
                     <div class="input-field-container">
                        ...
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="number" autocomplete="nope" data-qtype="number" data-qid="${e.screeningQuestionID}" ${e.isMandatory?"required":""} />
                        </label>
                        ${((d=e.helpText)==null?void 0:d.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `;case"Date":return`
                     <div class="input-field-container">
                        ...
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="date" autocomplete="nope" data-qtype="date" data-qid="${e.screeningQuestionID}" ${e.isMandatory?"required":""} />

                        </label>
                        ${((f=e.helpText)==null?void 0:f.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}; --shaz-showMoreShadowColor: ${s.config.showMoreShadowColor}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                            </div>
                        </div>
                        `||""}
                     ...
                     <div class="input-field-container">
                        <label>
                            <p class="sq-boolean-question">
                                <input tabindex="30" type="checkbox" autocomplete="nope" data-qtype="bool" data-qid="${e.screeningQuestionID}" ${e.isMandatory?"required":""}  />
                                ${e.question}
                                ${e.isMandatory?"*":""}
                            </p>
                        </label>
                        ${((l=e.helpText)==null?void 0:l.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `;case"List":{let p=(D=e.options)==null?void 0:D.map(O=>`<option value="${O.screeningQuestionOptionsID}">${O.label||O.option}</option>`);return`
                     <div class="input-field-container">
                        <p class="sq-list-question">${e.question}</p>
                        <select data-qtype="list" data-qid="${e.screeningQuestionID}" ${e.isMandatory?"required":""}>${(p==null?void 0:p.join(""))||""}</select>
                        ${((g=e.helpText)==null?void 0:g.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `}case"Multiselect Checkbox":{let p=(I=e.options)==null?void 0:I.map(O=>`<label><input tabindex="30" type="checkbox" autocomplete="nope" data-qtype="check-list" data-qid="${e.screeningQuestionID}" data-value="${O.screeningQuestionOptionsID}" ${e.isMandatory?"required":""} />${O.label||O.option}</label>`);return`
                     <div class="input-field-container">
                        <p class="sq-checklist-question">
                            ${e.question}
                            ${e.isMandatory?"*":""}
                        </p>
                        ${((v=e.helpText)==null?void 0:v.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `}case"Radio":{let p=(k=e.options)==null?void 0:k.map(O=>`<label class="sq-question-option"><input tabindex="30" type="radio" data-qtype="radio" name="${e.screeningQuestionID}" data-qid="${e.screeningQuestionID}" value="${O.screeningQuestionOptionsID}" ${e.isMandatory?"required":""} />${O.label||O.option}</label>`);return`
                     <div class="input-field-container">
                        <p class="sq-radio-question">
                            ${e.question}
                            ${e.isMandatory?"*":""}
                        </p>
                        ${((S=e.helpText)==null?void 0:S.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                        ...
                        `||""}
                        <div class="sq-opt-list">${(p==null?void 0:p.join(""))||""}</div>
                     </div>
                `}case"File":return`
                     <div class="input-field-container">
                        <label>
                        <p class="sq-list-question">
                            ...
                            <input tabindex="30" type="file" autocomplete="nope" data-qtype="file" data-qid="${e.screeningQuestionID}" ${e.isMandatory?"required":""} />
                        </label>
                        ${((h=e.helpText)==null?void 0:h.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `;case"Header":return`
                    <div class='screening-question-heading'>
                        <h3>${e.question}</h3>
                        ${((m=e.helpText)==null?void 0:m.length)>0&&`
                        <div class="sq-help-text" ${e.isHelpTextCollapse?"collapsible":""}>
                            <p class="text-main">${e.helpText||""}</p>
                            <div class="section-read-more" style="text-align: ${s.config.readMoreAlign}">
                                <a href="javascript: void(0);" class="button-show-more" data-rel="button-show-more">${s.config.showMoreHelpText}</a>
                ...
                `;default:return""}},this._pagingElements=e=>{let r=[];if(r.push(`<div data-rel="screening-pages" class="screening-pages-container" style="--shaz-text-align: ${s.config.PagingButtonAlignment}">`),e>0&&r.push(`<a href='javascript:void(0);' class="button-page-nav back" data-rel='screening-page-index' data-page='${e-1}'><span class="text">${s.config.pageBack||"Back"}</span></a>`),e<this._pages.length-1?r.push(`<a href='javascript:void(0);' class="button-page-nav forward" data-rel='screening-page-index' data-page='${e+1}'><span class="text">${s.config.pageNext||"Next"}</span></a>`):r.push(`<a href='javascript:void(0);' class="button-page-nav forward" data-rel='screening-apply'><span class="text">${s.config.applyTextBtn||"Apply"}</span></a>`),this._pages.length>1){let n=new Array(this._maxPage+1);for(let o=0;o<n.length;o++)n[o]=`<a href='javascript:void(0);' class="button-page-index ${o===e?"active":""}" data-rel='screening-page-index' data-page='${o}'></a>`;for(let o=n.length;o<this._pages.length;o++)n[o]=`<a href='javascript:void(0);' class="button-page-index disabled"></a>`;r.push(`<div class="sq-pages-index">${n.join("")}</div>`)}return r.push("</div>"),r.join("")},this._presentList=e=>{let r=e.map(o=>`
            <div>
        ...
        `;i(n).appendTo(i(u))},this._recordAnswers=()=>Promise.all(i(u).find("[data-rel=screening-fields] input, [data-rel=screening-fields] select").map((e,r)=>{var o;let n=i(r);switch(n.attr("data-qtype")){case"text":return n.val()?a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerText:n.val(),screeningTemplateID:a._screeningTemplateID}:delete a._answers[n.attr("data-qid")],Promise.resolve();case"number":return n.val()?a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerNum:parseInt(n.val()),screeningTemplateID:a._screeningTemplateID}:delete a._answers[n.attr("data-qid")],Promise.resolve();case"date":return n.val()?a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerDate:n.val(),screeningTemplateID:a._screeningTemplateID}:delete a._answers[n.attr("data-qid")],Promise.resolve();case"bool":return a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerBoolean:n.is(":checked")?1:0,screeningTemplateID:a._screeningTemplateID},Promise.resolve();case"list":return n.val()?a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerUUID:[n.val()],screeningTemplateID:a._screeningTemplateID}:delete a._answers[n.attr("data-qid")],Promise.resolve();case"check-list":{let d=n.attr("data-qid"),f=((o=a._answers[d])==null?void 0:o.answerUUID)||[];return n.is(":checked")?f.indexOf(n.attr("data-value"))<0&&f.push(n.attr("data-value")):f.indexOf(n.attr("data-value"))>=0&&f.splice(f.indexOf(n.attr("data-value")),1),f.length===0?delete a._answers[n.attr("data-qid")]:a._answers[d]={screeningQuestionID:n.attr("data-qid"),answerUUID:f,screeningTemplateID:a._screeningTemplateID},Promise.resolve()}case"radio":return n.is(":checked")&&(a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerUUID:[n.attr("value")],screeningTemplateID:a._screeningTemplateID}),Promise.resolve();case"file":{let d=n.get(0).files.length>0&&n.get(0).files[0];return d?a._readFile(d).then(f=>(a._answers[n.attr("data-qid")]={screeningQuestionID:n.attr("data-qid"),answerFile:f,answerFileName:d.name,screeningTemplateID:a._screeningTemplateID},Promise.resolve())):(delete a._answers[n.attr("data-qid")],Promise.resolve())}default:return Promise.resolve()}})),this._restoreAnswers=e=>{for(let r in a._answers){let n=a._answers[r];if(r=r.split(":")[0],!(e.indexOf(r)<0)){if(i(u).find(`[data-rel=screening-fields] input[data-qid=${r}]:visible, [data-rel=screening-fields] select[data-qid=${r}]:visible`).length===0){delete a._answers[r];continue}n.answerText||n.answerNum||n.answerDate?i(u).find(`[data-rel=screening-fields] input[data-qid=${r}]`).val(n.answerText||n.answerNum||n.answerDate):n.answerBoolean&&!n.answerUUID?i(u).find(`[data-rel=screening-fields] input[data-qid=${r}]`).attr("checked",!0):n.answerUUID?(typeof n.answerUUID=="string"&&(n.answerUUID=n.answerUUID.split(",")),n.answerUUID.forEach(o=>{i(u).find(`[data-rel=screening-fields] input[data-qid=${r}][data-value=${o.trim()}]`).attr("checked",!0),i(u).find(`[data-rel=screening-fields] input[data-qid=${r}][value=${o.trim()}]`).attr("checked",!0),i(u).find(`[data-rel=screening-fields] select[data-qid=${r}]`).val(o.trim())})):n.answerFile&&i(u).find(`[data-rel=screening-fields] input[data-qid=${r}]`).val(this._fileBlob(n.answerFile))}}},this._validate=()=>{let e=!0;return i(u).find("[data-rel=screening-fields] input[required], [data-rel=screening-fields] select[required]").each((r,n)=>{let o=i(n);switch(o.attr("data-qtype")){case"text":case"select":{let d=o.val();e=e&&d&&d.trim().length>0;break}case"bool":{e=e&&o.is(":checked");break}case"number":{let d=o.val();e=e&&d&&d.trim().length>0&&!isNaN(d);break}case"date":{let d=Date.parse(o.val());e=e&&!isNaN(d);break}case"radio":{e=e&&i(u).find(`[data-rel=screening-fields] input[type=radio][name=${o.attr("name")}]`).is(":checked");break}case"file":{e=e&&o.get(0).files.length>0;break}case"check-list":e=e&&i(u).find(`[data-rel=screening-fields] input[data-qtype=check-list][data-qid=${o.attr("data-qid")}]`).is(":checked")}}),e},this._readFile=e=>new Promise(r=>{let n=new FileReader;n.addEventListener("load",()=>{r(btoa(n.result))},!1),n.readAsBinaryString(e)}),this._fileBlob=(e,r="",n=512)=>{let o=atob(e),d=[];for(let l=0;l<o.length;l+=n){let D=o.slice(l,l+n),g=new Array(D.length);for(let v=0;v<D.length;v++)g[v]=D.charCodeAt(v);let I=new Uint8Array(g);d.push(I)}return new Blob(d,{type:r})},this._showEditor=()=>{let e=n=>{var o,d;return n.knockOutDate?new Date(n.knockOutDate).toLocaleDateString():n.knockOutList?(d=(o=n.options)==null?void 0:o.find(f=>f.screeningQuestionOptionsID===n.knockOutList))==null?void 0:d.option:n.knockOutText?n.knockOutText:n.knockOutNumber?n.knockOutNumber:n.knockOutBoolean!==null?n.knockOutBoolean?"Yes":"No":""},r=n=>{var o,d;return`
            <article data-rel="knockout" data-id="${n.screeningQuestionID}">
                <div class="row">
                ...
                <div class="row">
                    <label>
                        Message
                        <input type="text" data-rel="alert" placeholder="Sorry, we can accept you at this time." value="${n.alert||""}" />
                    </label>

                    <label>
                        Redirect (Leave blank for no redirect)
                        <input type="text" data-rel="redirect" placeholder="/reject" value="${n.redirect||""}" />
                    </label>
                </div>
                ...
                <div class="row">
                    <label>
                        Enable
                        <input type="checkbox" data-rel="enable" ${((o=n.alert)==null?void 0:o.length)>0||((d=n.redirect)==null?void 0:d.length)>0?'checked="checked"':""} />
                    </label>
                </div>
            </article>
        `};a._ko.length!==0&&t.config().then(n=>{let o=i(u).find("[data-dialog=knockout]");o.find("[data-rel=dialog-content]").empty().append(a._ko.map(f=>{var l;return r({...f,...(l=n==null?void 0:n.knockout)==null?void 0:l.find(D=>D.screeningQuestionID===f.screeningQuestionID)})})),i(u).find("[data-rel=screening-fields] .button-editor").remove(),i("<button />").text("Edit Knockout Handling").addClass("button-editor").on("click",function(){o.toggle()}).prependTo(i(u).find("[data-rel=screening-fields]"));let d=f=>{let l=i(f.target);i("<input />").attr({type:"checkbox","data-rel":"enable",checked:l.is(":checked")?"checked":null}).on("click",d).appendTo(l.parent()),l.remove()};o.find("[data-rel=enable]").on("click",d),o.find("[data-rel=button-submit]").on("click",function(){let f=(n==null?void 0:n.knockout)||[];o.find("[data-rel=dialog-content] article [data-rel=enable]").each((l,D)=>{let g=i(D).parents("article"),I=f.find(v=>v.screeningQuestionID===g.attr("data-id"));I&&f.splice(f.indexOf(I),1),i(D).is(":checked")&&f.push({screeningQuestionID:g.attr("data-id"),alert:g.find("[data-rel=alert]").val(),redirect:g.find("[data-rel=redirect]").val()})}),t.config().then(l=>t.config({...l,knockout:f})).then(()=>{n.knockout=f}),o.toggle()}),o.find("[data-rel=button-dismiss]").on("click",function(){o.toggle(),o.find("[data-rel=dialog-content]").empty().append(a._ko.map(f=>{var l;return r({...f,...(l=n==null?void 0:n.knockout)==null?void 0:l.find(D=>D.screeningQuestionID===f.screeningQuestionID)})})),o.find("[data-rel=enable]").on("click",d)})})},this._fetchAnswers=e=>b.site().then(r=>b.submit({action:"Get Screening Answers",siteID:r.siteID,jobApplicationID:e})).then(r=>{var o,d;let n=[];return(d=(o=r==null?void 0:r.response)==null?void 0:o.items)==null||d.forEach(f=>{n[f.screeningQuestionID]=f}),Promise.resolve(n)}),this._fetchQuestions().then(()=>{this._showQuestions(0,!1),s.inEditor&&this._showEditor()}),c&&this._fetchAnswers(c).then(e=>{let r=this._pages[0]||[];this._answers=e,this._restoreAnswers(r.map(n=>n.screeningQuestionID))}),this}let we=t=>{let c=ve(t,new URL(window.location).searchParams.get("jobApplicationID")),a=()=>{let o=e(),d=J(o),f=F;return d?P&&!o.cVFileContent?(alert(s.config.warningResume||"Please upload your resume"),Promise.reject()):f?b.firebase().validateEmail(o.eMail).then(()=>(o.candidateID=b.uuid(),s.config.showPassword||!s.config.useAnonymous?b.firebase().create(o.eMail,o.password).then(l=>Promise.resolve({...o,firebaseUserID:l.uid})).catch(l=>(y("hide"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),l.code==="auth/invalid-email"?alert(s.config.warningEmail||"Please use a valid email address"):l.code=="auth/email-already-in-use"&&s.config.showPassword&&window.confirm("The email is already an existing account please login instead.")&&(window.scrollTo({top:0,behavior:"smooth"}),i(u).find(".submitApplicationBtn").css("pointer-events","auto"),i(u).find(".applyText").show(),i(u).find(".fcLoader").removeClass("fcLoadingSeek")),Promise.reject(l))):Promise.resolve({...o,isValidated:!1}))).catch(()=>(y("hide"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),alert(s.config.warningEmail||"Please supply a valid email."),Promise.reject())):(y("hide"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),alert(s.config.warningPassword||"Please follow the password pattern!"),Promise.reject()):(y("hide"),alert(s.config.warningFields||"Please complete all required fields"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject())},e=()=>{var l,D,g,I,v,k,S;let o=i("#passwordInput").val()||"",d=i(u);s.config.showPassword||(o=b.uuid(),F=!0);let f={salutation:i(u).find("#salutation").val()||" ",firstName:((D=(l=d.find("#firstName").val())==null?void 0:l.trim())==null?void 0:D.replace(/["«»‘’‚‛“”„‟‹›❛❜❝❞❮❯〝〞〟＂❟❠⹂🙶🙷🙸＇]/g,""))||"",surname:((I=(g=d.find("#lastName").val())==null?void 0:g.trim())==null?void 0:I.replace(/["«»‘’‚‛“”„‟‹›❛❜❝❞❮❯〝〞〟＂❟❠⹂🙶🙷🙸＇]/g,""))||"",eMail:((v=d.find("#emailAddress").val())==null?void 0:v.trim())||"",phone:s.config.showPhoneNumber&&((k=d.find("#phoneNumber").val())==null?void 0:k.trim())||(s.config.requirePhone?"":void 0),mobile:s.config.showMobileNumber&&((S=d.find("#mobileNumber").val())==null?void 0:S.trim())||(s.config.requireMobile?"":void 0),password:o,isActive:!0,isValidated:!0,isSubscribed:X&&d.find("[data-rel=checkbox-subscribe]:checked").length>0,dudaSiteID:s.siteId,cVFileContent:T.resumeFile&&btoa(T.resumeFile),cVFileName:T.resumeName};return t&&s.config.enableTracing&&(!T.resumeFile||!T.coverLetterFile)&&(t.warn("file(s) missing",f),t.warn("file(s) missing (contains uploads)",T)),s.config.validatePhoneNumber&&(f.defaultPhoneCountry=s.config.defaultPhoneCountry||"AU",f.validatePhone=!0),f};t.sub(ee.submit,()=>{if(!N()){alert(s.config.warningNoJob||"No job selected!");return}let o=document.querySelector("input#resume[type=file]"),d=document.querySelector("input#cover[type=file]"),f=P||A||Object.keys(T).length||i(u).find("#uploadExisting").is(":checked");if(i(u).find(".fcLoader").addClass("fcLoadingSeek"),b.store("previousApplicationPage",null),!c.isValid){y("hide"),alert(s.config.warningQuestions||"Please complete all of the screening questions"),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}if(s.config.requireApproval===!0&&!i(u).find("[data-rel=field-approve]").is(":checked")){y("hide"),alert(s.config.approvalWarning||"Please confirm approval of the terms"),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}let l=(g,I)=>(I==null?void 0:I.length)>0&&I||(g==null?void 0:g.length)>0&&g||"",D=i(u).find("input.invalid").map((g,I)=>`- ${l(i(I).attr("data-name"),i(I).attr("placeholder"))}`).toArray();if(D.length>0){y("hide"),alert(`${s.config.warningValidation||"Please correct the following:"}
${D.join(`
`)}`),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}c.answers().then(g=>{let I=c.knockout(g);if((I==null?void 0:I.length)>0){t.config().then(h=>{I.forEach(m=>{var O,L,x;let p=(O=h==null?void 0:h.knockout)==null?void 0:O.find(R=>R.screeningQuestionID===m.screeningQuestionID);if(((L=p==null?void 0:p.alert)==null?void 0:L.length)>0&&alert(p.alert),((x=p==null?void 0:p.redirect)==null?void 0:x.length)>0){let R=s.inEditor?`/site/${C}${p.redirect}?preview=true&insitepreview=true&dm_device=desktop`:p.redirect;window.location=R}})}),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}let v=h=>{var p,O;let m=e();return J(m)?P&&!m.cVFileContent?(y("hide"),alert(s.config.warningResume||"Please upload your resume"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject()):(m.cVFileContent=(p=m.cVFileContent)!=null?p:h.candidate.cVFileContent,m.cVFileName=(O=m.cVFileName)!=null?O:h.candidate.cVFileName,m.phone=m.phone||h.candidate.phone||null,m.mobile=m.mobile||h.candidate.mobile||null,delete m.dudaSiteID,delete m.defaultPhoneCountry,delete m.validatePhone,b.submit({action:"Edit Candidate Info",...h.candidate,...m}).then(L=>L.status&&b.user(!0)||Promise.reject())):(y("hide"),alert(s.config.warningFields||"Please complete all required fields"),i(u).find(".fcLoader").removeClass("fcLoadingSeek"),Promise.reject())},k=h=>{var fe,ue,le,be;let m={referralSource:_.searchParams.get("utm_source")||b.session("referralSource"),referralMedium:_.searchParams.get("utm_medium")||b.session("referralMedium"),referralTerm:_.searchParams.get("utm_term")||b.session("referralTerm"),referralCampaign:se()||_.searchParams.get("utm_campaign")||b.session("referralCampaign"),referralContent:_.searchParams.get("utm_content")||b.session("referralContent")},p={jobID:N(),screeningAnswers:g,screeningTemplateID:W()||(c==null?void 0:c._screeningTemplateID)||void 0,...m};if(f){if(i("#uploadExisting").is(":checked")&&((fe=h==null?void 0:h.candidate)!=null&&fe.cVFileContent))p.resumeFile=(ue=h==null?void 0:h.candidate)==null?void 0:ue.cVFileContent,p.resumeFileName=(le=h==null?void 0:h.candidate)==null?void 0:le.cVFileName;else try{let w=ne(o,P,"resume");p.resumeFile=w&&btoa(T.resumeFile),p.resumeFileName=w&&T.resumeName,p.resumeUpdatedOn=w&&new Date}catch(w){i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}try{let w=ne(d,A,"coverLetter");p.coverLetterFile=w&&btoa(T.coverLetterFile),p.coverLetterFileName=w&&T.coverLetterFileName,p.coverLetterUpdatedOn=w&&new Date}catch(w){i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}if(o&&P&&!p.resumeFile){alert(s.config.warningResume||"Please upload your resume");return}if(d&&A&&!p.coverLetterFile){alert(s.config.warningCoverLetter||`Please upload your cover letter, or tick the "I don't have cover letter"`);return}}s.config.showGender&&(p.customField1=i("#gender").val()),s.config.showAboriginal&&(p.customField2=i("#aboriginal").is(":checked")),s.config.enableTracing&&(p.session=b.uuid(),t.log("apply",{candidate:h,application:p}));let O;try{O=JSON.parse(b.store("currentJobViewed"))}catch(w){}let L=(O==null?void 0:O.data)||{},x,G=[ae()?null:oe()||L[s.config.brochureField||"customField2"],L.applicationURL].find(w=>typeof w=="string"&&w.length>0);if((G==null?void 0:G.length)>0)x=G;else if(x=`/${E}`,s.config.includeLastSearch&&(O!=null&&O.data)){let w=[];(s.config.redirectJobField||[]).forEach(pe=>{let V=O.data[pe.field];(V==null?void 0:V.length)>0&&w.push(`${pe.field}=${encodeURIComponent(V)}`)}),w.length&&(x=`/${E}?${w.join("&")}`)}let de={action:"Apply Job",dudaSiteID:s.siteId,application:p,candidate:h};if(p.resumeFile||p.coverLetterFile){b.submit(de).then(()=>{y("stop"),M.clearKeys(),window.location.href=x}).catch(()=>{y("hide"),alert(s.config.warningApply||"Sorry, your application could not be submitted. Please try again."),i(u).find(".fcLoader").removeClass("fcLoadingSeek")});return}let ye=((be=b._site)==null?void 0:be.RegionalUrl)||"https://shazamme.io/Job-Listing/src/php/regional/actions";try{fetch(ye,{method:"POST",keepalive:!0,headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:JSON.stringify(de)})}catch(w){}y("stop"),M.clearKeys(),window.location.href=x},S=e();if(!J(S)){y("hide"),alert(s.config.warningFields||"Please complete all required fields"),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}if(P&&!S.cVFileContent){y("hide"),alert(s.config.warningResume||"Please upload your resume"),i(u).find(".fcLoader").removeClass("fcLoadingSeek");return}S.candidateID=b.uuid(),k(S)})}),i(u).find(".eye").on("click",function(){i(this).toggleClass("fa-eye-slash").toggleClass("fa-eye").siblings("input").attr("type",function(d,f){return f==="text"?"password":"text"})}),s.config.validatePhoneNumber&&typeof libphonenumber=="object"&&i(u).find("input[type=telephone]").on("change",function(){let o=i(this);if(o.removeClass("invalid"),o.val().trim().length!==0)try{let d=libphonenumber.parsePhoneNumber(o.val(),s.config.defaultPhoneCountry||"AU");!d.country||!d.isValid()?o.addClass("invalid"):o.val(d.formatInternational())}catch(d){o.addClass("invalid")}}),b.store("applicationURL",window.location.href),re().then(o=>{r(o),o!=null&&o.data?o.data.jobID===s.config.jobID&&t.pub("job-application-fixed-job",o.data.jobID,!0):s.inEditor||(i(".shmApplicationMainContainer").hide(),i(".section-no-job-message").show())});function r(o){var l;let d=oe()||((l=o==null?void 0:o.data)==null?void 0:l[s.config.brochureField||"customField2"]);if(!ae()||!(typeof d=="string"&&d.length>0))return;let f=i(u).find("[data-rel=brochure-link]");f.length===0&&(f=i('<div class="brochure-link" data-rel="brochure-link"></div>').appendTo(i(u).find(".shmApplicationMainContainer").first())),f.empty().append(i('<a target="_blank" rel="noopener noreferrer"></a>').attr("href",d).text(s.config.brochureLinkText||"View the candidate brochure"))}let n=o=>{if(o!=null&&o.isNew){ie({salutation:" ",firstName:o.firstName,surname:o.lastName,eMail:o.email,phone:"",mobile:"",isNew:!0});return}let d=o==null?void 0:o.candidate;ie(d),d!=null&&d.candidateID&&me(d.candidateID).then(f=>{if(f)if(confirm(s.config.duplicateWarning||"DUPLICATE WARNING MESSAGE")){let l=window.location.href.includes(C)?`/site/${C}/${K}?preview=true&insitepreview=true&dm_device=desktop`:`/${K}`;window.location.href=l}else window.location.reload()})};b.user().then(o=>{n(o)}),t.sub("site-auth",o=>{n(o)})};i("head").append(i('<link rel="stylesheet" type="text/css" href="https://sdk.shazamme.io/css/fontawesome/css/fontawesome.min.css" crossorigin="anonymous" />')).append(i('<link rel="stylesheet" type="text/css" href="https://sdk.shazamme.io/css/fontawesome/css/regular.min.css" crossorigin="anonymous" />')),window.__shazLoadScript("https://sdk.shazamme.io/js/shazamme-1.0.3.min.js").then(function(){Promise.all([b.ready(s.siteId,s.page),b.script("https://sdk.shazamme.io/js/plugin/libphonenumber/1.10.54/plugin.min.js")]).then(()=>{we(b.register("application-screening-form",s,!0))})})}return Ce(_e);})();
(function(){
