 ...
 * Build 5fd918c11513. Registers window.ShazammeWidget["<name>"].
 */
...
var __shazWidgetExport = (() => {
  ...
  // dist/.gen/site-config.index.js
  var site_config_index_exports = {};
  __export(site_config_index_exports, {
  ...
  function legacyController(ctx) {
    ...
    const enableFileUploads = (w) => {
      ...
      ux.el.find("[data-rel=dialog][data-dialog=file] input[type=file]").on("change", function() {
        let f = this.files[0];
        ...
        files.push(f);
        ux.el.find("[data-rel=dialog][data-dialog=file] [data-rel=list-files]").append(
      ...
      ux.el.find("[data-rel=dialog][data-dialog=file]").on("click", "[data-rel=button-save]", function() {
      ...
      let showTypes = (t) => {
        let el = ux.el.find("[data-rel=dialog][data-dialog=file] [data-rel=candidate-file-type]").empty().append(t == null ? void 0 : t.map((i) => `<option value="${i.siteCandidateFileTypeID}">${i.candidateFileType}</option>`));
      ...
      let showFiles = (u) => {
        ...
        return (data.inEditor && Promise.resolve(JSON.parse('{"response":{"items":[{"data": {"candidateFileID": "e5c290f3-5e28-456a-b00f-0686a1b28731","candidateFileName": "my-test-file.pdf","siteCandidateFileTypeID": "80743118-14f6-42be-9441-4a12c2d2af8d"}}]}}')) || shazamme.site().then(
          ...
          ux.el.find("[data-rel=dialog][data-dialog=file] [data-rel=candidate-file-type]").on("change", function() {
            const t = $(this).val();
            ux.el.find("[data-rel=candidate-file-collection]").empty().append(
              docs.filter((i) => data.inEditor || i.siteCandidateFileTypeID === t).map((i) => $(`
              ...
              if (data.inEditor) {
                ux.el.find("[data-rel=dialog][data-dialog=file] [data-rel=list-files]").empty();
                el.parents("[data-rel=dialog][data-dialog=file]").fadeOut();
                return;
              }
              ux.showLoading();
              const selected = (b64) => {
          ...
          if (docs.length > 0) {
            ux.el.find("[data-rel=dialog][data-dialog=file] .tab-bar").removeClass("hidden").on("click", "[data-rel=tab]", function() {
              let tab = $(this);
              ux.el.find(".tab-bar [data-rel=tab]").removeClass("active");
              tab.addClass("active");
              ux.el.find(`[data-rel=tab]`).hide();
              ux.el.find(`button[data-rel=tab], [data-rel=tab][data-tab=${tab.attr("data-tab")}]`).show().removeClass("hidden");
    ...
    const enableIconPicker = (w) => {
      ...
      let search = (s, q) => {
        $.ajax({
          ...
          var _a, _b;
          ux.el.find("[data-rel=icon-list]").empty().append(
            (_b = (_a = r == null ? void 0 : r.data) == null ? void 0 : _a.search) == null ? void 0 : _b.map((i) => `<i class="${s} fa-${i.id}" title="${i.label}" data-id="${i.id}" data-code="${i.unicode}" data-set="${s}" />`)
          );
        });
      };
      w.sub(Message.iconPicker, () => {
        ux.el.find("[data-rel=icon-list]").empty().on("click", "i", function() {
          let icon = $(this);
          ...
          ux.el.find("[data-rel=dialog][data-dialog=icons]").fadeOut();
        });
        ux.el.find("[data-rel=icon-search]").val("").on("keyup", function() {
          if (this._debounce) {
        ...
        ux.el.find("[data-rel=dialog][data-dialog=icons]").css({
          display: "block",
          opacity: 100
    ...
    const alertDialog = (o) => {
      let el = $(`
        <div class="dialog shazamme-dialog alert" data-rel="dialog" data-dialog="alert">
            <div class="dialog-content">
                <p class="title">${(o == null ? void 0 : o.title) || "Alert"}</p>
                <div class="message">${(o == null ? void 0 : o.message) || ""}</div>

                <div class="dialog-buttons">
                    <button class="button-main" data-rel="action-close"><span class="text">${(o == null ? void 0 : o.buttonText) || "OK"}</span></button>
                </div>
            </div>
        </div>
    `).copyCSS(_alertDialogT, null, ["display"]).on("click", "[data-rel=action-close]", function() {
        try {
          o == null ? void 0 : o.onClose();
        } catch (e) {
        }
        $(this).parents("[data-dialog=alert]").remove();
      });
      el.find(".dialog-content").copyCSS(_alertDialogT == null ? void 0 : _alertDialogT.find(".dialog-content"));
    ...
    const toast = (m, t = 2500) => new Promise((res, rej) => {
      ...
      if (el.length === 0) {
        ...
        `).copyCSS(_toastT, null, ["display", "opacity"]).on("click", ".close", function() {
      ...
      const show = (t2) => {
        if (t2) {
          el.css({
            opacity: 100
          }).find(".text").first().text(t2.message);
          _toastTimeout = setTimeout(() => {
            show(_toast.shift());
          }, t2.to);
        } else {
          _toastTimeout = void 0;
          el.css({
            opacity: 0
          });
    ...
    let main = (w) => {
      ...
      if (data.inEditor) {
        ...
        ux.el.find("[data-rel=button-show-tools]").on("click", function() {
          ux.el.find(".site-configuration-toolbar").toggle().removeClass("hidden");
        });
        enableFileUploads(w);
        enableIconPicker(w);
        ...
        shazamme.site().then((s) => {
          ux.el.find("[data-info-site-id]").text(s.siteID).on("click", function() {
      ...
      if (data.config.jobFieldMap) {
        let m = data.config.jobFieldMap.filter((i) => i.jobFieldFrom && i.jobFieldTo).map((i) => `${i.jobFieldFrom}-${i.jobFieldTo}`);
      ...
      w.sub(Message.toast, (m) => {
        if (data.inEditor) {
          toast("This is a toast message", 1e4);
        } else {
          toast((m == null ? void 0 : m.message) || "", (m == null ? void 0 : m.timeout) || 2500);
        }
      });
      w.pub(Message.ready, {}, true);
      shazamme.bag(Message.ready, true);
    };
    ux.el.addClass("site-config").children(":first").show();
    ux.loadScript("https://sdk.shazamme.io/js/shazamme-1.0.2.min.js").then(() => shazamme.ready(data.siteId, data.page)).then(() => shazamme.script("https://sdk.shazamme.io/js/plugin/jquery-copycss/jquery.copycss.min.js")).then(() => {
  ...
  return __toCommonJS(site_config_index_exports);
})();
(function(){
  ...
  if (typeof window !== 'undefined') {
    ...
    window.ShazammeWidget["site-config"] = controller;
